	...
	content: [
		...
		<Grid key="5" cols={1} gap={4} className="mb-8">
			<FeatureCard icon={FileText} title="Step 1: git add" subtitle="Blob creation in the object store" theme="teal">
				<p className="text-sm text-teal-700 dark:text-teal-100/75">
					Git hashes each staged file's content into a <strong className="text-teal-700 dark:text-teal-400">Blob</strong> object inside
					<code className="text-teal-700 dark:text-teal-300">.git/objects</code>, then records that blob in the staging area (index).
				</p>
				<p className="mt-3 text-xs text-teal-700 dark:text-teal-300/70">
			...
			>
				<p className="text-sm text-cyan-700 dark:text-cyan-100/75">
					Git creates a <strong className="text-cyan-700 dark:text-cyan-400">Tree</strong> from the index to map filenames to blob hashes,
					then writes a <strong className="text-cyan-700 dark:text-cyan-400">Commit</strong> object that points to that tree and to the
					previous commit as its parent.
				</p>
				<p className="mt-3 text-xs text-cyan-700 dark:text-cyan-300/70">
			...
			>
				<p className="text-sm text-violet-700 dark:text-violet-100/75">
					The current branch ref, such as <code className="text-violet-700 dark:text-violet-300">.git/refs/heads/main</code>, is updated to
					point at the new commit SHA. Branches are just lightweight named pointers, not separate copies of your project.
				</p>
				<p className="mt-3 text-xs text-violet-700 dark:text-violet-300/70">

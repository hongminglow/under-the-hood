...
export const observabilityTopic: Topic = {
	...
	content: [
		<p key="1">
			<strong>Monitoring</strong> tells you <em>when</em> something is broken. <strong>Observability</strong> tells you{" "}
			<em>why</em>. In distributed systems with 50+ microservices, you can't SSH into every server to read logs. You
		...
		<Table
			...
			rows={[
				["Logs", "What exactly happened?", "Too verbose at scale, hard to correlate"],
				["Metrics", "Is the system healthy? How fast?", "No context on individual failures"],
				["Traces", "Where did this request slow down?", "Expensive to capture for every request"],
			]}
		/>,
		<Callout key="4" type="tip" title="The Golden Signals (Google SRE)">
			Google's SRE book defines <strong>4 golden signals</strong> to monitor: <strong>Latency</strong> (how fast),{" "}
			<strong>Traffic</strong> (how much), <strong>Errors</strong> (how often things fail), <strong>Saturation</strong>{" "}
			(how full). If your dashboard covers these 4 metrics for every service, you have production-grade monitoring.
		</Callout>,
		<Callout key="5" type="info" title="OpenTelemetry: The Unified Standard">
			<strong>OpenTelemetry</strong> (OTel) is the industry-standard vendor-neutral framework that unifies logs,
			metrics, and traces into one SDK. Instrument once, export to any backend (Datadog, Grafana, Jaeger). Every major

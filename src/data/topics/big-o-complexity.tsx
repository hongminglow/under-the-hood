...
export const bigOComplexityTopic: Topic = {
	...
	content: [
		<p key="1">
			<strong>Big O notation</strong> describes how an algorithm's runtime grows <em>relative to the input size</em>. It
			doesn't measure actual milliseconds — it measures the <strong>rate of growth</strong>. An <code>O(n)</code>{" "}
			algorithm that takes 1ms per element will take 1 second for 1,000 elements, but an <code>O(n²)</code> algorithm
			will take <strong>16 minutes</strong>.
		</p>,
		...
		<Table
			...
			rows={[
				...
				["O(n log n)", "Linearithmic", "~10,000 ops", "Merge sort, quicksort avg"],
				["O(n²)", "Quadratic", "1,000,000 ops", "Nested loops, bubble sort"],
				["O(2ⁿ)", "Exponential", "~10³⁰⁰ ops", "Brute-force subsets"],
				["O(n!)", "Factorial", "∞ for practical n", "Permutations, TSP brute force"],
			]}
		/>,
...
while (lo <= hi) {
  ...
  arr[mid] < target ? lo = mid + 1 : hi = mid - 1;
		...
		<Callout key="6" type="warning" title="The 'Drop Constants' Rule">
			Big O drops constants and lower-order terms. <code>O(2n + 100)</code> simplifies to <code>O(n)</code>.{" "}
			<code>O(n² + n)</code> simplifies to <code>O(n²)</code>. But in real-world performance, constants matter — an{" "}
			<code>O(n)</code> algorithm with a massive constant can be slower than <code>O(n log n)</code> for practical input
			sizes.
		</Callout>,
	],
};

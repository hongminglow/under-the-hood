  ...
  "retryCount": 0
    ...
    <Table
      ...
      rows={[
        ...
        ["Message Retention", "Deleted after consumption", "Retained for days/weeks", "Max 14 days", "Not retained (fire-and-forget)"],
    ...
    <Grid key="16" cols={2} gap={6} className="my-8">
      <Card title="Use RabbitMQ When...">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Priority queues:</strong>&nbsp;High-priority messages jump the line</li>
          <li><strong>Small-medium scale:</strong>&nbsp;10k-50k messages/sec</li>
        </ul>
      </Card>
      <Card title="Use Kafka When...">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Multiple consumers:</strong>&nbsp;Analytics, ML, search all read same stream</li>
          <li><strong>Event sourcing:</strong>&nbsp;Rebuild state from event log</li>
        </ul>
      </Card>
      <Card title="Use AWS SQS When...">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Simple queues:</strong>&nbsp;No complex routing needed</li>
          <li><strong>FIFO guarantees:</strong>&nbsp;SQS FIFO queues (300 msg/sec limit)</li>
        </ul>
      </Card>
      <Card title="Use MQTT When...">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Lightweight:</strong>&nbsp;Runs on microcontrollers (ESP32)</li>
          <li><strong>QoS levels:</strong>&nbsp;Fire-and-forget, at-least-once, exactly-once</li>
        </ul>
      </Card>
    </Grid>,
    ...
    <p key="18" className="mb-4">
      Kafka isn't a queue—it's an <strong>immutable, distributed commit log</strong>. Messages are never deleted after consumption. Instead, each consumer tracks its <strong>offset</strong>&nbsp;(position in the log). This enables powerful patterns impossible with traditional queues.
    </p>,
    <CodeBlock
      key="19"
...
// - "ml-training" at offset 0 (reading from beginning)`}
    ...
    <p key="21" className="mb-4">
      In 2015, Uber was using RabbitMQ for trip events. As they scaled to 1M trips/day, RabbitMQ couldn't keep up. Messages were lost during broker restarts. They migrated to Kafka and unlocked new capabilities.
    </p>,
...
async function handleOrder(message) {
  ...
  try {
    ...
    <Grid key="28" cols={2} gap={6} className="my-8">
      <Card title="Pros">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Traffic smoothing:</strong>&nbsp;Handle spikes without crashing</li>
          <li><strong>Async processing:</strong>&nbsp;API responds instantly</li>
          <li><strong>Retry logic:</strong>&nbsp;Built-in retry and DLQ</li>
        </ul>
      </Card>
      <Card title="Cons">
        <ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
          ...
          <li><strong>Debugging difficulty:</strong>&nbsp;Async errors are harder to trace</li>
          <li><strong>Operational overhead:</strong>&nbsp;Kafka requires Zookeeper, tuning</li>
          <li><strong>Cost:</strong>&nbsp;Managed services (AWS MSK) can be expensive</li>
        </ul>
      </Card>
    </Grid>,
    ...
    <p key="30" className="mb-4">
      In microservices, you can't use database transactions across services. <strong>Sagas</strong>&nbsp;use message queues to coordinate multi-step workflows with compensating transactions.
    </p>,
    <CodeBlock
      key="31"
...
// - shipping.create → ShippingService listens`}
    ...
    <Callout key="32" type="tip" title="Pro Tip: Use Message Schemas">
      As your system grows, enforce message schemas with tools like <strong>Avro</strong>&nbsp;(Kafka) or <strong>JSON Schema</strong>&nbsp;(RabbitMQ). This prevents breaking changes when producers update message formats. Schema registry (Confluent) versions schemas and validates compatibility.
    </Callout>,
    ...
    <Callout key="34" type="warning" title="Avoid Queues For...">
      <ul className="text-sm space-y-2 list-disc pl-4">
        <li><strong>Synchronous workflows:</strong>&nbsp;User needs immediate response (payment confirmation). Use direct API calls.</li>
        <li><strong>Low latency requirements:</strong>&nbsp;Real-time gaming, video calls. Use WebSockets or UDP.</li>
        <li><strong>Simple CRUD apps:</strong>&nbsp;If you're just reading/writing to a database, queues add unnecessary complexity.</li>
        <li><strong>Strong consistency needed:</strong>&nbsp;Financial transactions requiring ACID. Use database transactions, not eventual consistency.</li>
      </ul>
    </Callout>,
  ],
};

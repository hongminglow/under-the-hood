...
export const onlineMultiplayerArchitectureTopic: Topic = {
  ...
  content: [
    <p key="1">
      Building an online PvP (Player vs Player) game like <em>Valorant</em>,{" "}
      <em>League of Legends</em>, or <em>Call of Duty</em> is fundamentally
      different from building a web application. Standard web servers are entirely
      stateless and react to events when you ask them to. A game server is an
      always-running, mathematically precise simulation of a virtual world that
      must synchronize state across dozens of clients in less than 30
      milliseconds.
    </p>,
    ...
    <Grid key="3" cols={2} gap={6}>
      <Card title="Web / HTTP Server" description="Stateless, Request/Response">
        <ul className="list-disc pl-5 mt-2 text-sm text-foreground/80 space-y-1">
          ...
          <li><strong>Scale:</strong> Horizontally scalable. Put a Load Balancer in front of 100 identical clones.</li>
        </ul>
      </Card>
      <Card title="Dedicated Game Server" description="Stateful, Continuous Simulation">
        <ul className="list-disc pl-5 mt-2 text-sm text-foreground/80 space-y-1">
          ...
          <li><strong>Scale:</strong> Strictly vertical per match. Client A and Client B *must* connect to the exact same physical server instance.</li>
        </ul>
      </Card>
    </Grid>,

    <Callout key="4" type="info" title="Why UDP instead of TCP?">
      If a packet containing your character's position is lost over TCP, the protocol temporarily stops all incoming traffic, demands a resend, and waits. In real-time PvP, old positional data is entirely useless. If we missed where you were at frame 10, we don't care anymore — we need frame 11! UDP blasts data constantly without waiting for acknowledgements, preventing the "Head-of-line blocking" delay found in TCP.
    </Callout>,
    ...
    <p key="6">
      A dedicated server doesn't "react" to player input the moment it arrives.
      Instead, it collects all inputs in a buffer and processes them in discrete
      time steps called <strong>Ticks</strong>. If a server has a 64Hz Tick Rate
      (like CS:GO), the server computes the state of the world 64 times every
      second (~every 15.6ms).
    </p>,
    ...
    <p key="9">
      When a million players hit "Find Match", they aren't talking to game servers
      yet. They are talking to standard web infrastructure. The orchestrator must
      find 10 compatible players, spin up a dedicated server for them, and hand
      them the IP address.
    </p>,
    ...
    <p key="12">
      It takes around 50ms for a packet to reach the server, and 50ms to get back. A
      100ms round-trip feels sluggish and unplayable to humans. To fix this, game
      clients flat-out lie to you using two core concepts:
    </p>,
    ...
    <p key="15">
      ...
      <strong>In games, the server's RAM is the ONLY source of truth.</strong>{" "}
      Querying a database during a 16ms tick window is catastrophic. If the game
      waits on a database lock, the entire match stutters for all players.
    </p>,
    ...
    <Grid key="16" cols={2} gap={6}>
      ...
      <Card title="The 'Cold Path' (Asynchronous)" description="Account Persistence">
        <p className="mt-2 text-sm text-foreground/80 leading-relaxed">
          ...
          worker slowly writes this to a persistent database (PostgreSQL / DynamoDB).
        </p>
      </Card>
    </Grid>,

    <Callout key="17" type="warning" title="What happens when a game server crashes?">
      Because nothing is saved mid-match to the central DB, if the EC2 node
      hosting the match shuts down, the match completely evaporates. The players
      are kicked back to the lobby, and no one gains or loses MMR, because the
      RAM state is permanently gone.
    </Callout>,
    ...
    <p key="19">
      Game servers prioritize <strong>Single-Threaded CPU Performance</strong> over
      all else. Cloud providers often struggle with this because their hardware is
      optimized for multi-tenant, multi-core virtualization.
    </p>,

    <ul key="20" className="list-disc pl-5 space-y-2 mb-8">
      <li>
        <strong>High Clock Speeds (GHz):</strong> A single game match physics
        simulation cannot easily be parallelized across multiple cores. It runs on
        one main thread. A 4.0GHz core provides much smoother tick rates than two
        2.5GHz cores.
      </li>
      <li>
        <strong>Bare Metal Servers:</strong> Because hypervisors (VMs) introduce
        minute jitter, highly competitive games (like CS2 and Valorant) run on bare
        metal hardware. Riot Games famously built their own massive global ISP network (Riot
        Direct) just to bypass traditional BGP routing and guarantee minimal pings to
        their bare-metal data centers.
      </li>
      <li>
        <strong>Memory:</strong> They use very little memory compared to enterprise

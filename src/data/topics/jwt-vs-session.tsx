...
export const jwtVsSessionTopic: Topic = {
	...
	content: [
		...
		<Grid key="verify-grid" cols={3} gap={6} className="mb-8">
			<FeatureCard icon={KeyRound} title="1. Header (Base64)" subtitle="Algorithm metadata" theme="cyan">
				<p className="text-sm font-mono text-cyan-700 dark:text-cyan-100/75 break-all">eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9</p>
				<p className="text-xs mt-2 border-t border-cyan-800/40 pt-2 text-cyan-700 dark:text-cyan-300/70">
					Tells the server which algorithm to use (e.g., HS256).
				</p>
			</FeatureCard>
			<FeatureCard icon={Database} title="2. Payload (Base64)" subtitle="Visible claims data" theme="amber">
				<p className="text-sm font-mono text-amber-700 dark:text-amber-100/75 break-all">
					eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0
				</p>
				<p className="text-xs mt-2 border-t border-amber-800/40 pt-2 text-amber-700 dark:text-amber-300/70">
					Contains the actual user data and expiration time.
				</p>
			</FeatureCard>
			<FeatureCard icon={ShieldCheck} title="3. Signature (Hashed)" subtitle="The anti-tamper lock" theme="emerald">
				<p className="text-sm font-mono text-emerald-700 dark:text-emerald-100/75 break-all">SflKxwRJSMeKKF..._adQssw5c</p>
				<p className="text-xs mt-2 border-t border-emerald-800/40 pt-2 text-emerald-700 dark:text-emerald-300/70">
					The cryptographic lock that prevents tampering.
				</p>
			</FeatureCard>
		</Grid>,
		<FeatureCard key="verify-math" icon={ShieldCheck} title="The Mathematics of Verification" subtitle="Why stateless verification works" theme="emerald">
...
const signature = HMACSHA256(
  ...
  process.env.JWT_SECRET
			...
			/>
			<p className="text-sm text-emerald-700 dark:text-emerald-100/75 mt-4">
			...
			<ul className="list-disc pl-5 mt-4 text-sm text-emerald-700 dark:text-emerald-100/70 space-y-2">
				<li>
		...
		<Grid key="4" cols={2} gap={6} className="my-12">
			<FeatureCard icon={ShieldAlert} title="LocalStorage Nightmare for JWTs" subtitle="The common unsafe pattern" theme="rose">
			...
			<FeatureCard icon={ShieldCheck} title="The HttpOnly Cookie Saving Grace" subtitle="The safer browser-side default" theme="emerald">

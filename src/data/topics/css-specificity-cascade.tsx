...
/* RESULT: RED ✅
   ...
   The long selector is not "targeting the parent" — the word 'a' at the
   end means it applies directly to the <a> element. The ancestors
   (#header, .nav, ul…) are just context conditions, not the target.   */`}
    />,
...
/* RESULT: BLUE ✅
   ...
   These are NOT compared by specificity score. The inherited value is
   immediately set aside. Any explicit rule at any specificity — even
   0-0-0-1 — beats a bare inherited value. So blue wins. The 0-2-0-1
   score on #wrapper #nav ul is IRRELEVANT to the <a>.                 */`}
    />,
    ...
    <Callout key="43" type="tip" title="When Specificity Ties: Source Order Wins">
      When two rules carry the identical specificity score, the final tie-breaker is <strong>source order</strong> — the rule
      that appears last in the browser's single merged stylesheet wins (file boundaries do not exist in the cascade). In a
      bundled app that order is set by your <strong>JS import order</strong>, so the practical rule is: import third-party CSS
      first and your own CSS last, and your styles win ties without needing <code>!important</code>. For a guarantee that
      survives accidental import reordering, wrap third-party CSS in a low-priority <code>@layer</code> — un-layered styles
      always beat layered ones.
    </Callout>,

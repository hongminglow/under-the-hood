...
export const tcpIpTopic: Topic = {
  ...
  content: [
    ...
    <p key="2a" className="mb-4">
      While the <strong>TCP/IP Model</strong> is what's used in practice (4 or 5 layers), the <strong>OSI Model</strong> (7 layers) is the standard reference for understanding how data moves through the stack.
    </p>,
    <Callout key="FunNote" type="info" title="Is there more? (Layer 8 & 9)">
      The OSI model stops at Layer 7, but engineers often joke about <strong>Layer 8 (The User)</strong> and <strong>Layer 9 (Politics/Organization)</strong> as the real bottlenecks in networking!
    </Callout>,
    <Table
      key="3"
    ...
    <p key="3b" className="mb-4 text-sm">
      When you call `fetch()`:
    </p>,
    <ul key="3c" className="list-decimal pl-5 text-sm text-muted-foreground space-y-2 mb-8">
      <li><strong>Data (Layer 7)</strong> is created (JSON).</li>
      <li>It's <strong>encrypted (Layer 6)</strong> and a session is managed (Layer 5).</li>
      <li>It's split into <strong>segments (Layer 4)</strong> where TCP adds sequence numbers.</li>
      <li>It's put into <strong>packets (Layer 3)</strong> with an IP address.</li>
      <li>It's wrapped in <strong>frames (Layer 2)</strong> for the Wi-Fi/Ethernet chip.</li>
      <li>It's pulsed out as <strong>bits (Layer 1)</strong> over the wire.</li>
    </ul>,

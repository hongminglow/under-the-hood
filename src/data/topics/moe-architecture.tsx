...
export const moeArchitectureTopic: Topic = {
  ...
  description:
    "How models efficiently scale parameters by routing tokens only to specialized sub-networks.",
  tags: ["ai", "architecture", "moe"],
  icon: "Brain",
  content: [
    <p key="intro" className="mb-4">
      Historically, LLMs were <strong>Dense Models</strong>—every parameter in
      the brain is activated for every single word generated. While effective,
      scaling this approach requires an absurd amount of compute (FLOPs). A{" "}
      <strong>Mixture of Experts (MoE)</strong> flips this paradigm. Instead of
      one massive dense network, the model is composed of a Router and several
    ...
    <Flow
      ...
      steps={[
        { title: "Input Token", description: "The word e.g., 'Python'" },
        {
          ...
          description:
            "A linear layer calculates probabilities determining which experts are best suited.",
        },
        {
          title: "Expert Activation",
          description:
            "Only top-K experts (usually 2 out of 8) run the computation.",
        },
        {
          title: "Weighted Sum",
          description:
            "The outputs of the activated experts are combined for the final prediction.",
        },
      ]}
    />,
    ...
    <Grid key="grid" cols={2} gap={6}>
      <Card title="Dense Model (e.g., Llama 3)">
        <ul className="text-sm space-y-2 list-disc pl-4 mb-4">
          ...
          <li>
            <strong>Example:</strong> A 70B dense model uses 70B parameters for
            every word.
          </li>
        </ul>
      </Card>
      <Card title="MoE Model (e.g., Mixtral 8x7B)">
        <ul className="text-sm space-y-2 list-disc pl-4 mb-4">
          ...
          <li>
            High VRAM required (all experts fit in memory), but{" "}
            <strong>low compute</strong>.
          </li>
          <li>
            <strong>Example:</strong> An MoE with 8x7B experts uses only 2
            experts per token, performing the compute of a ~13B model.
          </li>
        </ul>
      </Card>
    </Grid>,
    ...
    <p key="modern-sub" className="mb-4">
      As you move into the tier of "frontier models", engineering heavily favors
      MoE because you can scale the model's total "knowledge capacity" without
      skyrocketing the per-token inference cost. Here is how modern models stack
      up:
    </p>,
    <Table
      key="table"
    ...
    <MistakeCard
      ...
      solution={
        <>
          <strong>MoE saves Compute, NOT Memory.</strong> Even though only 2
          experts are used <em>per token</em>, the router could pick any expert
          at any time. Therefore,{" "}
          <strong>all experts must physically reside in VRAM</strong>{" "}
          constantly. A 47B MoE requires exactly the same VRAM as a 47B Dense
          model.
        </>
      }
    />,
    ...
    <Grid key="pros-cons-grid" cols={2} gap={6} className="my-8">
      <Card title="Pros">
        <ul className="text-sm text-slate-400 space-y-2 list-disc pl-4">
          ...
          <li>
            <strong>Massive Knowledge Capacity:</strong> Total parameter count
            can be scaled to trillions without breaking real-time latency.
          </li>
          <li>
            <strong>Lower Training/Serving Cost:</strong> TFLOPs per token
            remain bound to the active experts, saving immense
            electricity/compute costs for API providers.
          </li>
          <li>
            <strong>Specialization:</strong> Experts naturally organize
            themselves into specializations (e.g., an expert that fires mainly
            for Python syntax, another for French translation).
          </li>
        </ul>
      </Card>
      <Card title="Cons">
        <ul className="text-sm text-slate-400 space-y-2 list-disc pl-4">
          ...
          <li>
            <strong>Expert Collapse / Load Balancing:</strong> The router can
            get "lazy" and only route to Expert #1, starving others. Complex
            training loss penalties are required to force even load balancing.
          </li>
          <li>
            <strong>Higher Cold-Start Latency:</strong> Loading massive 1.8T
            parameter weights into GPU memory takes significantly longer than
            standard dense models.
          </li>
          <li>
            <strong>Communication Bottlenecks:</strong> In multi-GPU setups
            (tensor parallelism), bouncing tokens between GPUs to find their
            required expert limits scaling efficiency.
          </li>
        </ul>
      </Card>
    </Grid>,

...
export const corsTopic: Topic = {
	...
	description: "Why your perfectly fine API fails entirely in the browser, but works perfectly fine in Postman.",
	tags: ["security", "frontend", "headers", "api-design"],
	icon: "Ban",
	content: [
		<p key="1">
			The most deeply hated error message in all of frontend development:{" "}
			<code>Blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present</code>.
		</p>,
		<p key="2" className="mt-4">
			As developers, we constantly assume CORS is our backend "blocking" the request, but{" "}
			<strong>CORS is entirely enforced by the Browser!</strong>&nbsp;It is a critical security protection to prevent a
			malicious website (<code>hackers.com</code>) from making silent background API requests to{" "}
			<code>your-bank.com</code> using your currently logged-in cookies.
		</p>,
		...
		<FeatureCard key="5" icon={ShieldAlert} title="The Attack Without CORS Protection" subtitle="Why the browser intervenes" theme="rose">
			<ol className="text-sm text-rose-100/75 space-y-2">
				...
				<li>
					2. You visit <code>evil-site.com</code> (maybe from a phishing email).
				</li>
				<li>
					3. Evil site runs JavaScript:{" "}
					<code>
						fetch('https://bank.com/transfer', &#123; method: 'POST', body: JSON.stringify(&#123; to: 'attacker',
						amount: 10000 &#125;) &#125;)
					</code>
				</li>
				<li>
					4. <strong>Without CORS</strong>: Your browser automatically sends your <code>bank.com</code> cookies with the
					request. Money transferred. You're broke.
				</li>
				<li>
					5. <strong>With CORS</strong>: Browser blocks the request because <code>bank.com</code> didn't whitelist{" "}
		...
		<p key="8" className="mb-4">
			An origin is the combination of <Highlight variant="primary">protocol</Highlight> +{" "}
			<Highlight variant="primary">domain</Highlight> + <Highlight variant="primary">port</Highlight>. If <em>any</em>
			&nbsp;of these differ, it's a cross-origin request.
		</p>,
		<Table
			key="9"
		...
		<p key="11" className="mb-4">
			If a React app at <code>localhost:3000</code> attempts to POST JSON data to <code>api.my-startup.com</code>, they
			are on two completely different domains (Origins). The browser performs a <strong>preflight check</strong>
		...
		<p key="15" className="mb-4">
			Not all cross-origin requests trigger a preflight. <strong>Simple requests</strong>&nbsp;skip the OPTIONS check
		...
		<Callout key="17" type="info" title="Why Simple Requests Exist">
			Simple requests mirror what HTML forms can do naturally (<code>&lt;form method="POST"&gt;</code>). Since forms
			have existed since the 1990s, browsers allow these without preflight for backward compatibility.
		</Callout>,
...
app.post('/api/transfer', (req, res) => {
  ...
  res.json({ success: true });
		...
		<p key="23" className="mb-4">
			When sending cookies or Authorization headers, you must set <code>credentials: 'include'</code> on the frontend{" "}
			<strong>AND</strong>&nbsp;<code>Access-Control-Allow-Credentials: true</code> on the backend. But here's the
			catch:
		</p>,
		<Callout key="24" type="warning" title="The Wildcard Restriction">
			If <code>Access-Control-Allow-Credentials: true</code> is set, you <strong>CANNOT</strong>&nbsp;use{" "}
			<code>Access-Control-Allow-Origin: *</code>. You must specify the exact origin. This is a security requirement to
...
origin: /^http:\\/\\/localhost:\\d+$/`}
		...
		<Callout key="31" type="warning" title="CORS is Browser-Only">
			Postman is not a web browser. It is a desktop application. It couldn't care less about Cross-Origin policies, so
			it directly hits your API and retrieves the JSON flawlessly. The CORS error purely lives inside Chrome, Safari,
			and Firefox. This is why your API "works in Postman but not in the browser."
		</Callout>,
		<Table
			key="32"
		...
		<Grid key="34" cols={2} gap={6} className="my-8">
			<Card title="1. Proxy Server (Dev)">
				<p className="text-sm text-muted-foreground mb-2">In development, use a proxy to avoid CORS entirely.</p>
				<CodeBlock
					language="json"
...
// Now fetch('/api/users') goes to localhost:5000`}
			...
			<Card title="2. Same-Origin Deployment">
				<p className="text-sm text-muted-foreground mb-2">Deploy frontend and backend on the same domain.</p>
				<CodeBlock
					language="text"
...
No CORS needed - same origin!`}
		...
		<Card key="35" title="3. JSONP (Legacy, Avoid)">
			<p className="text-sm text-muted-foreground mb-2">
				Before CORS, developers used JSONP (JSON with Padding) by injecting <code>&lt;script&gt;</code> tags. This is{" "}
				<strong>insecure</strong>&nbsp;and deprecated. Never use it.
			</p>
		</Card>,
		...
		<Card key="37" title="Story 1: The Wildcard Disaster">
			<p className="text-sm text-muted-foreground mb-2">
				A startup set <code>Access-Control-Allow-Origin: *</code> with <code>credentials: true</code> on their payment
				API. Attackers created phishing sites that made authenticated requests, draining user accounts. The fix?
				Whitelist only their official domain. Cost: $2M in refunds.
			</p>
		</Card>,
		<Card key="38" title="Story 2: The Preflight Performance Hit">
			<p className="text-sm text-muted-foreground mb-2">
				A high-traffic API was getting <strong>double the requests</strong>&nbsp;due to preflight OPTIONS calls. They
				added <code>Access-Control-Max-Age: 86400</code> to cache preflight responses for 24 hours. Result: 50%
				reduction in OPTIONS traffic, saving $10K/month in server costs.
			</p>
		</Card>,
		<Card key="39" title="Story 3: The Mobile App Confusion">
			<p className="text-sm text-muted-foreground mb-2">
				A team spent 3 days debugging CORS errors in their React Native app. The issue? React Native doesn't enforce
				CORS (it's not a browser). The real problem was a typo in the API URL. Always check if CORS is actually relevant
				to your environment.
			</p>
		</Card>,
		...
		<Grid key="41" cols={2} gap={6} className="my-8">
			<FeatureCard icon={CheckCircle2} title="DO" subtitle="Safe CORS defaults" theme="emerald">
				<ul className="text-sm text-emerald-100/75 space-y-2">
					...
					<li>
						• Use <code>Access-Control-Max-Age</code> to reduce preflight overhead
					</li>
			...
			<FeatureCard icon={AlertTriangle} title="DON'T" subtitle="Common ways to weaken it" theme="rose">
				<ul className="text-sm text-rose-100/75 space-y-2">
					...
					<li>
						• Trust the <code>Origin</code> header without validation
					</li>
					<li>• Disable CORS entirely (use proxy instead)</li>
					<li>
						• Allow <code>null</code> origin (used by file:// and sandboxed iframes)
					</li>
		...
		<Callout key="42" type="info" title="The Simple Backend Fix">
			Backend developers must install a CORS middleware (like the <code>cors</code> package in Express) and explicitly
			whitelist the exact frontend domains they wish to allow. Setting it to <code>*</code> is an epic vulnerability for
			authenticated systems, as it lets ANY domain hit your API.
		</Callout>,

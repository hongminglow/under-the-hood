...
export const dockerContainersTopic: Topic = {
	...
	content: [
		<p key="1">
			For years, developers faced the infamous <strong>"It works on my machine!"</strong> problem. You write code on
			your Windows laptop, it works perfectly, but when you send it to the Linux production server, it crashes because
			of wrong Node.js versions, missing packages, or different OS configurations.
		</p>,
		<p key="2" className="mt-4">
			<strong>Docker</strong> was created to solve this. Instead of just sending your code, you send your code{" "}
			<em>along with</em> the exact environment it needs to run. Docker packages your app and all its dependencies into
			a standardized puzzle piece called a <Highlight variant="primary">Container</Highlight>.
		</p>,
		...
		<Grid key="4" cols={2} gap={6} className="my-6">
			<Card title="Absolute Consistency" description="No More Surprises">
				<p className="text-sm mt-2 text-muted-foreground">
					Since the container includes the code, the runtime (like Node or Python), and the operating system details, it
					behaves exactly the same on your laptop, on your co-worker's MacBook, and on the AWS production cloud.
				</p>
			</Card>
			<Card title="Speed & Efficiency" description="Lighter than VMs">
				<p className="text-sm mt-2 text-muted-foreground">
					Unlike traditional Virtual Machines (VMs) that boot up entirely new heavy operating systems, containers share
					the host's OS. They are tiny, fast, and start in milliseconds.
				</p>
			</Card>
		</Grid>,
		...
		<Card key="6" title="The Blueprint vs. The House">
			<ul className="list-disc pl-5 space-y-2 mt-2 text-sm text-muted-foreground">
				<li>
					<strong>Docker Image:</strong> The static blueprint. It's an immutable file that contains your source code,
					libraries, dependencies, and tools.
				</li>
				<li>
					<strong>Docker Container:</strong> The running instance of that Image. You can run many identical containers
					from a single Image.
				</li>
			</ul>
		</Card>,
		...
		<p key="8" className="mb-6">
			Docker is the backbone of modern Continuous Integration and Continuous Deployment (CI/CD). Here is how a typical
			pipeline looks when you push code to GitHub:
		</p>,
		<Step key="9" index={1}>
			<strong>Build:</strong> The CI server (like GitHub Actions) reads your{" "}
			<Highlight variant="primary">Dockerfile</Highlight>—a simple instruction file—and builds a fresh Docker Image
			containing your updated code.
		</Step>,
		<Step key="10" index={2}>
			<strong>Ship (Push):</strong> The CI server uploads this newly built Image to a central storage area called a
			Registry (like Docker Hub or AWS Elastic Container Registry).
		</Step>,
		<Step key="11" index={3}>
			<strong>Run (Deploy):</strong> Your production servers download the newest Image from the Registry, stop the old
			containers, and spin up new containers exactly recreating your fresh application environment.
		</Step>,
...
CMD ["npm", "start"]
      `}
		...
		<p key="multistage-desc" className="mb-4">
			A naive Dockerfile includes build tools, dev dependencies, and source files — bloating the image to over 1GB.{" "}
			<strong>Multi-stage builds</strong>&nbsp;let you compile in one stage and copy only the final artifacts to a
...
# Result: 150MB instead of 1.2GB`}
		...
		<Callout key="multistage-tip" type="success" title="Why This Works">
			The final image only contains the <code>node:20-alpine</code> base + production dependencies + compiled code. All
			build tools (TypeScript compiler, webpack, etc.) are left behind in the builder stage.
		</Callout>,
		...
		<p key="compose-desc" className="mb-4">
			Instead of manually starting containers one by one, <strong>Docker Compose</strong>&nbsp;lets you define
			multi-container applications in a single YAML file.
		</p>,
		<CodeBlock
			key="compose-code"
...
volumes:
  postgres_data:`}
		...
		<p key="compose-usage" className="text-sm text-muted-foreground mt-4">
			Start everything with: <code>docker-compose up</code>
			<br />
...
COPY . .`}
		...
		<Callout key="best-practices" type="tip" title="Docker Best Practices Checklist">
			...
			<br />✅ Use specific image tags (not <code>:latest</code>)<br />✅ Scan images for vulnerabilities (

...
export const oauth2OidcTopic: Topic = {
  ...
  content: [
    ...
    <p key="7" className="mb-4">
      OAuth 2.0 defines <strong>4 grant types</strong>&nbsp;— each designed for a specific client type and trust level. Using the wrong flow is a <strong>security vulnerability</strong>.
    </p>,
    <Table
      key="8"
...
const { access_token, id_token } = await response.json();`}
    ...
    <p key="14" className="mb-4">
      For clients that <strong>can't store a client secret</strong>&nbsp;(mobile apps, SPAs). PKCE (Proof Key for Code Exchange, pronounced "pixy") prevents authorization code interception attacks.
    </p>,
    <Flow
      key="15"
      steps={[
...
const response = await fetch('https://oauth2.googleapis.com/token', {
  ...
  body: new URLSearchParams({
    ...
    <Callout key="17" type="info" title="Why PKCE Works">
      Even if an attacker intercepts the authorization code, they can't exchange it for a token without the original <code>code_verifier</code>, which only exists on the legitimate client device.
    </Callout>,
    ...
    <p key="19" className="mb-4">
      No user involved — <strong>service-to-service auth</strong>. Your backend talks to another API (Stripe, AWS, Twilio) without human interaction.
    </p>,
    <CodeBlock
      key="20"
...
// Use token for API calls (cron jobs, microservices, CI/CD)`}
    ...
    <Callout key="22" type="warning" title="Officially Deprecated in 2019">
      For years, developers returned the Token directly to the React app in the browser URL (<code>#access_token=...</code>). This was horrific for security (URL history logs, malicious scripts, browser extensions). The OAuth Working Group officially deprecated Implicit Flow. <strong>Use Authorization Code + PKCE instead</strong>.
    </Callout>,
    <Table
      key="23"
    ...
    <Grid key="27" cols={2} gap={6} className="my-8">
      <Card title="When to Use Each Flow">
        <ul className="text-sm text-muted-foreground space-y-2">
          ...
          <li>• <strong>Microservices</strong>: Client Credentials</li>
          <li>• <strong>Cron Jobs</strong>: Client Credentials</li>
        </ul>
      </Card>
    ...
    <Callout key="32" type="tip" title="Quick Decision Tree">
      ...
      <strong>Need to know who the user is?</strong>&nbsp;→ OIDC (OAuth + ID Token)<br/>
      <strong>Have a backend?</strong>&nbsp;→ Authorization Code<br/>
      <strong>No backend (SPA/Mobile)?</strong>&nbsp;→ Authorization Code + PKCE<br/>

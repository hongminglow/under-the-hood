	...
	content: [
		...
		<Grid key="3" cols={2} gap={6} className="mb-8">
			<FeatureCard icon={Package} title="Webpack" subtitle="Bundle-based development server" theme="teal">
				<p className="mb-4 text-teal-700 dark:text-teal-100/80">
				...
				<p className="text-teal-700 dark:text-teal-100/70">
		...
		<Grid key="5" cols={1} gap={6} className="mb-8">
			<FeatureCard icon={Gauge} title="1. esbuild" subtitle="Vite's unfair speed advantage" theme="amber">
				<p className="mb-4 text-amber-700 dark:text-amber-100/80">
					...
					engine parse speeds. Vite completely sidesteps this by using <strong className="text-amber-700 dark:text-amber-400">esbuild</strong>,
...
export default defineConfig({
  plugins: [react()]
		...
		<Table
			key="7"
			headers={["Metric", "<span class='text-teal-700 dark:text-teal-400'>Webpack</span>", "<span class='text-amber-700 dark:text-amber-400'>Vite</span>"]}
			rows={[
		...
		<Callout key="8" type="warning" title="What About Production?">
			...
			production bundler, it intelligently delegates this task to <strong className="text-amber-700 dark:text-amber-400">Rollup</strong>{" "}

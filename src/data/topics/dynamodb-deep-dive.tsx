...
export const dynamoDbDeepDiveTopic: Topic = {
	...
	content: [
		<p key="1" className="mb-4">
			DynamoDB is Amazon's fully managed, serverless key-value and document database. It underpins Amazon.com's shopping
			cart, Prime Video, and hundreds of AWS services internally — and it guarantees{" "}
			<strong>single-digit millisecond latency at literally any scale</strong>. Understanding <em>how</em> it achieves
			that is a masterclass in distributed systems engineering.
		</p>,
		...
		<p key="3" className="mb-4">
			DynamoDB is the productized, managed evolution of the internal <strong>Amazon Dynamo</strong> system described in
			the legendary 2007 paper <em>"Dynamo: Amazon's Highly Available Key-Value Store"</em>. That paper introduced the
			world to consistent hashing rings, vector clocks, and tunable consistency — concepts that now underpin Cassandra,
			Riak, and most modern distributed databases.
		</p>,
		<Callout key="4" type="info" title="Why Amazon Built Dynamo">
			In 2004, a single RDBMS bottleneck caused Amazon.com's checkout cart to go down during peak traffic. Engineers
			realized relational databases — with their strict consistency guarantees and single-master writes — fundamentally
			could not handle Amazon's scale. Dynamo was born from the requirement:{" "}
			<strong>"always writable, always available, even during network partitions."</strong>
		</Callout>,
		...
		<p key="7" className="mb-4">
			DynamoDB operates a massive fleet of storage nodes arranged in a <strong>consistent hash ring</strong>. When you
			write an item, DynamoDB hashes your <code>Partition Key</code> value and maps it to a position on the ring. That
			position determines which physical storage node(s) store your data. This means:
		</p>,
		<div key="8" className="space-y-3 mb-6">
			<Step index={1}>
				<strong>Deterministic routing:</strong> Given a partition key, the router can compute exactly which node holds
				the data in O(1) — no directory lookup, no central coordinator.
			</Step>
			<Step index={2}>
				<strong>Automatic sharding:</strong> Data is automatically distributed across thousands of internal partitions
				without you managing shard keys or rebalancing.
			</Step>
			<Step index={3}>
				<strong>Elastic scaling:</strong> Adding a node only moves a slice of the ring — the rest of the data stays put.
				No full resharding needed.
			</Step>
		</div>,
		...
		<p key="10" className="mb-4">
			DynamoDB's storage engine is built on <strong>Log-Structured Merge Trees (LSM trees)</strong> — the same structure
			used by Cassandra, RocksDB, and LevelDB. Unlike B-Trees (which PostgreSQL uses and which require random disk
		...
		<Grid key="11" cols={2} gap={6} className="my-6">
			<Card title="How LSM Tree Writes Work">
				<ol className="text-sm text-muted-foreground space-y-1 list-decimal list-inside">
					...
					<li>
						MemTable is periodically flushed to immutable disk files (<strong>SSTables</strong>)
					</li>
		...
		<p key="13" className="mb-4">
			Every write to DynamoDB is <strong>synchronously replicated to 3 Availability Zones</strong> before it's
			acknowledged as successful. Each replica runs its own LSM storage engine. A write is confirmed only when at least
			2 of 3 replicas commit it to their WAL — guaranteeing no data loss even if an entire AZ goes dark.
		</p>,
		<CodeBlock
			key="14"
...
Entire round trip: ~1-5ms`}
		...
		<p key="16" className="mb-4">
			DynamoDB has no declarative query language like SQL. You get exactly what you design for. The data model is built
			around two keys:
		</p>,
		<Table
			key="17"
...
// This requires a Scan (full table read) OR a Global Secondary Index (GSI)`}
		...
		<p key="20" className="mb-4">
			When you need to query by a non-primary-key attribute, you define a <strong>Global Secondary Index (GSI)</strong>.
			DynamoDB asynchronously maintains a separate, fully replicated copy of your table projected on a different key
			pattern. It's essentially a second table maintained transparently.
		</p>,
		<Callout key="21" type="warning" title="GSI Consistency Trade-off">
			GSI reads are <strong>eventually consistent</strong> — there's a small replication lag between a write to the base
			table and when that write becomes visible in the GSI (typically milliseconds, but not zero). For strict
			read-after-write consistency, you must query the base table directly using its primary key.
		</Callout>,
		...
		<p key="25" className="mb-4">
			DynamoDB's write path is intentionally simple. There are no foreign key checks, no join validations, no constraint
			evaluations beyond primary key uniqueness. Every write just needs to:
		</p>,
		<div key="26" className="space-y-3 mb-6">
			<Step index={1}>
				<strong>Authenticate & authorize</strong> the IAM request (pre-computed, cached).
			</Step>
			<Step index={2}>
				<strong>Route</strong> to the correct partition via consistent hash (O(1)).
			</Step>
			<Step index={3}>
				<strong>Write to MemTable + WAL</strong> on leader node (in-memory, sequential write).
			</Step>
			<Step index={4}>
				<strong>Replicate</strong> to 2 follower nodes for quorum (parallel network calls).
			</Step>
			<Step index={5}>
				<strong>Return ACK</strong> to client. Total: single-digit milliseconds, regardless of table size.
			</Step>
		</div>,
		<Callout key="27" type="success" title="On-Demand Capacity: Instant Auto-Scaling">
			In <strong>On-Demand mode</strong>, DynamoDB automatically scales write throughput to handle sudden spikes — no
			pre-provisioning needed. Amazon.com uses this for Black Friday, where writes jump 100× in minutes. DynamoDB has
			handled sustained workloads of <strong>89 million requests per second</strong> (documented at AWS re:Invent).
		</Callout>,
		...
		<Callout key="32" type="tip" title="The Single-Table Design Pattern">
			...
			orders, products) in one table, differentiated by a naming convention on PK/SK (e.g., <code>PK="USER#42"</code>,{" "}
			<code>SK="ORDER#2024-05-01#xyz"</code>). This eliminates cross-table joins entirely since all related data lives
			in one partition, dramatically reducing latency and cost. Libraries like <strong>ElectroDB</strong> and{" "}
			<strong>Dynamoose</strong> provide a schema layer on top of this pattern.
		</Callout>,
	],
};

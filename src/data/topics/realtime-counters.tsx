...
export const realtimeCountersTopic: Topic = {
	...
	content: [
		<p key="intro">
			You've seen it: an npm package page where the download count visibly ticks upward. A Twitch stream where the
			viewer count updates every few seconds. A YouTube premiere where the live thumbs-up counter climbs in real time. A
			donation tracker that jumps the moment someone gives. These look instantaneous and magical — but the engineering
			behind them is a carefully-designed pipeline of <strong>event capture → aggregation → push delivery</strong>. The
			naive "one DB write per event" approach would destroy any production database at scale.
		</p>,
		...
		<p key="naive-sub" className="mb-6">
			The first instinct most developers have: every time someone runs <code>npm install react</code>, send a POST
			request to the registry that does <code>UPDATE packages SET downloads = downloads + 1</code>. This works at zero
			scale and completely collapses at real scale.
		</p>,
		<Grid key="naive-grid" cols={2} gap={6} className="mb-8">
			<Card title="What Actually Happens at Scale">
				<ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
					<li>
						<strong>React gets downloaded ~50M times per week.</strong> That's roughly{" "}
						<strong>400 writes per second</strong>, sustained, 24/7.
					</li>
					<li>
						Each write takes a <strong>row-level lock</strong> on the <code>packages</code> table to safely increment
						the counter.
					</li>
					<li>
						With 400 concurrent lock contentions per second on <em>one row</em>, queries back up. Timeouts cascade. The
						database tips over.
					</li>
					<li>
						And React is just one package. There are over <strong>2 million packages</strong> on npm.
					</li>
				</ul>
			</Card>
			<Card title="The Fundamental Problem">
				<ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
					<li>
						<strong>Hot Row Contention:</strong> A single counter row becomes a global bottleneck. Every INCR serializes
						behind the last.
					</li>
					<li>
						<strong>Write Amplification:</strong> For every 1 user "event", you generate 1 synchronous DB write — at
						scale this sum is enormous.
					</li>
					<li>
						<strong>Precision Overkill:</strong> Does it matter if the count shows 49,999,821 vs 50,000,000? For a
						download ticker, approximate accuracy seconds behind is perfectly acceptable.
					</li>
				</ul>
			</Card>
		</Grid>,
		...
		<p key="pipeline-sub" className="mb-6">
			Production-grade real-time counters are never a direct DB write. They always follow a multi-stage pipeline that
			separates high-throughput <strong>event ingestion</strong> from lower-frequency <strong>storage writes</strong>{" "}
			and the final <strong>live push</strong> to browsers.
		</p>,
		<Flow
			key="flow"
			steps={[
				{
					title: "1. User Action",
					description: "npm install react, a Twitch viewer joins, a donation is submitted.",
				},
				{
		...
		<p key="npm-sub" className="mb-4">
			npm (owned by GitHub/Microsoft) has published parts of their infrastructure. Downloads are NOT counted on install.
			They are counted at the <strong>CDN layer</strong> — when the registry CDN serves the tarball (the{" "}
			<code>.tgz</code> file) to your machine.
		</p>,
		<Grid key="npm-grid" cols={2} gap={6} className="mb-8">
			<Card title="What Gets Counted">
				<ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
					<li>
						A "download" is recorded every time the CDN successfully delivers a package tarball (the physical{" "}
						<code>.tgz</code>).
					</li>
					<li>
						<strong>CI/CD pipelines count.</strong> This is why React's download numbers are astronomically higher than
						its actual user base — every CI run reinstalls packages.
					</li>
					<li>
						<strong>Not real time.</strong> The counter you see on npmjs.com is updated roughly{" "}
						<strong>once per 24 hours</strong>. It is an aggregated daily number, not a live ticker.
					</li>
				</ul>
			</Card>
			<Card title="The CDN Log Pipeline">
				<ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
					<li>CDN edge nodes (Cloudflare, Fastly) log every tarball delivery as a structured access log line.</li>
					<li>
						These logs stream into a <strong>data pipeline</strong> (Kafka / Kinesis) where a processing job counts by
						package name + date.
					</li>
					<li>
						The aggregated counts are written to a <strong>ClickHouse</strong> or BigQuery table — an analytics database
						optimized for massive columnar writes, not transactional row locks.
					</li>
				</ul>
			</Card>
		</Grid>,
		<Callout key="npm-callout" type="info" title="It Looks Faster Than It Is">
			The "blazingly fast" counter you see on some package docs sites (like bundlephobia or similar) is actually
			fetching from a <strong>cached API endpoint</strong> (e.g.{" "}
			<code>api.npmjs.org/downloads/point/last-week/react</code>), not from a live stream. That endpoint returns a
			number that is hours old — but React's count is so large it still looks dramatic when displayed.
		</Callout>,
		...
		<p key="live-sub" className="mb-4">
			These are a completely different beast. A Twitch viewer count or a donation ticker{" "}
			<em>genuinely does update in seconds</em>. The full pipeline involves a persistent connection from your browser
		...
		<p key="redis-sub" className="mb-4">
			Almost every platform at scale uses Redis for high-frequency counter operations before flushing to a persistent
			store. The key command is <code>INCR</code>, which is atomic, single-threaded, and runs in nanoseconds.
		</p>,
...
app.get('/live/downloads/:pkg', (req, res) => {
  ...
  req.on('close', () => redisSub.unsubscribe());
		...
		<p key="frontend-trick-sub" className="mb-4">
			As discussed, maintaining genuine sub-second updates for a massive audience destroys server capacity. The industry
			standard trick is to poll every 5-10 seconds, and use client-side javascript to smoothly{" "}
			<strong>interpolate (animate)</strong> between the old number and the new number natively in the browser.
		</p>,
...
export default function AnimatedCounter({ targetValue }: { targetValue: number }) {
  ...
  return <div className="font-mono text-4xl">{displayValue.toLocaleString()}</div>;
		...
		<Callout key="final-tip" type="tip" title="The Counter Illusion">
			Many "real-time" counters you see on the web are <strong>animated interpolations</strong> of infrequently-fetched
			numbers. YouTube's like counter doesn't jump from 10,432 to 10,500 instantly — it smoothly animates between polled
			values every few seconds using a client-side counter that increments locally. The animation is designed to{" "}
			<em>create the feeling</em> of liveness, even when the underlying data update frequency is much slower than it
			appears.
		</Callout>,
	],
};

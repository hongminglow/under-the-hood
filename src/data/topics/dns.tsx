...
export const dnsTopic: Topic = {
  ...
  content: [
    <p key="intro-1">
      DNS (Domain Name System) is often described as "the internet's phonebook." But that analogy barely scratches the surface. To a DevOps engineer, DNS is a <strong>hierarchical, globally-distributed, time-cached lookup system</strong> that every single network request on earth depends on. Let's trace the entire lifecycle of one URL from keystroke to HTML.
    </p>,
    ...
    <p key="flow-sub" className="text-muted-foreground mb-6">
      Before a single byte of your webpage can be fetched, the browser must hunt down the raw IP address for the hostname. This is the <strong>DNS Resolution</strong> phase. It can involve up to 5 different actors.
    </p>,
    <Flow
      key="dns-flow"
      steps={[
        {
          ...
          description: "First stop. Chrome/Firefox keep their own in-memory DNS cache (separate from the OS). If the record hasn't expired, resolution is over in microseconds.",
        },
        {
          ...
          description: (
            <span>
              ...
              <code className="break-all">C:\Windows\System32\drivers\etc\hosts</code> (Windows) first, then its own DNS cache. Critical for local dev overrides.
            </span>
          ),
        },
        {
          title: "3. Recursive Resolver",
          description: "Usually your ISP's server or a public one (1.1.1.1 Cloudflare, 8.8.8.8 Google). This is the 'detective' that does all the hunting on your behalf.",
        },
        {
          title: "4. Root Name Servers",
          description: "13 logical root servers (operated by ICANN, NASA, etc.). They don't know the IP — they point the resolver to the correct TLD server (.com, .io, .my).",
        },
        {
          title: "5. TLD Name Servers",
          description: "Managed by registries: Verisign runs .com, PIR runs .org. They know which Authoritative server is responsible for your specific domain.",
        },
        {
          title: "6. Authoritative Nameserver",
          description: "The final source of truth. This is where YOU set your DNS records (Cloudflare DNS, AWS Route 53, GoDaddy DNS). It returns the actual IP.",
        },
      ]}
    />,
    ...
    <p key="steps-sub" className="text-muted-foreground mb-6">
      Let's say a user types <code>https://app.myshop.com</code> into Chrome. Their ISP's Recursive Resolver has a cold cache. Here is every network hop that happens before Chrome renders a single pixel:
    </p>,
    <Card key="steps-card" className="mb-8">
      <div className="space-y-4">
        <Step index={1}><strong>Browser checks its own DNS cache.</strong> Miss. TTL expired on last visit.</Step>
        <Step index={2}><strong>OS checks <code className="break-all">/etc/hosts</code> file.</strong> No manual override entry. OS checks its own socket cache. Miss.</Step>
        <Step index={3}><strong>OS asks the Recursive Resolver</strong> (<code>1.1.1.1</code>): "What's the IP for <code>app.myshop.com</code>?" — The resolver checks its own massive cache. Cold miss.</Step>
        <Step index={4}><strong>Resolver asks a Root Name Server:</strong> "Who handles <code>.com</code>?" Root responds: "Ask Verisign at <code className="break-all">a.gtld-servers.net</code>."</Step>
        <Step index={5}><strong>Resolver asks the .com TLD Server (Verisign):</strong> "Who is authoritative for <code>myshop.com</code>?" TLD responds: "Ask Cloudflare at <code className="break-all">erin.ns.cloudflare.com</code>." (These are the NS records your Registrar set.)</Step>
        <Step index={6}><strong>Resolver asks YOUR Authoritative Nameserver (Cloudflare/Route53):</strong> "What is the A record for <code>app.myshop.com</code>?" Cloudflare responds: "It's <code>104.21.55.12</code>, TTL=300."</Step>
        <Step index={7}><strong>Resolver caches</strong> <code>app.myshop.com → 104.21.55.12</code> for 300 seconds and hands the answer back to your browser.</Step>
        <Step index={8}><strong>Browser initiates TCP connection</strong> to <code>104.21.55.12:443</code>. DNS is now completely DONE. The rest of the request is a separate concern (TCP Handshake → TLS → HTTP).</Step>
      </div>
    </Card>,
    ...
    <p key="after-dns-sub" className="text-muted-foreground mb-6">
      The IP is resolved. Now the browser makes the actual HTTP(S) request. From a DevOps perspective, what that IP address points to determines the entire infrastructure chain:
    </p>,
    <Flow
      key="request-flow"
      steps={[
        {
          title: "TCP Handshake",
          description: "Browser opens a raw TCP socket to the IP on port 443. 3-way SYN → SYN-ACK → ACK. Adds ~1 RTT of latency.",
        },
        {
          title: "TLS Handshake",
          description: "Client Hello → Server presents SSL Certificate → browser validates against CA (Let's Encrypt, DigiCert) → shared secret negotiated. Adds ~1-2 RTT.",
        },
        {
          title: "CDN Edge Node",
          description: "Often the IP belongs to Cloudflare/CloudFront. The request hits the nearest PoP (Point of Presence) — NOT your origin server. CDN checks its cache.",
        },
        {
          title: "Load Balancer",
          description: "If the CDN misses, request passes to your origin. An AWS ALB or Nginx load balancer distributes it across your fleet of app servers.",
        },
        {
          title: "Application Server",
          description: "Your Node.js / Django / Go server receives the HTTP request, runs business logic, queries the database.",
        },
        {
          title: "Response Flows Back",
          description: "App → Load Balancer → CDN (caches it) → TLS encrypted → TCP → browser. Browser renders HTML. Total time: 50–500ms.",
        },
      ]}
    />,
    ...
    <Table
      ...
      rows={[
        ...
        ["CNAME", "Maps hostname → another hostname", "app.myshop.com → shop.vercel.app", "Alias to a PaaS (Vercel, Heroku, AWS). Cannot be used on apex root domain."],
        ["NS", "Delegates zone to nameservers", "erin.ns.cloudflare.com", "Set by Registrar to point to your DNS Provider. THE most important record."],
        ["MX", "Mail exchange routing", "aspmx.l.google.com (priority 1)", "Point @yourdomain.com email to Google Workspace or Zoho."],
        ["TXT", "Arbitrary text", "v=spf1 include:_spf.google.com ~all", "Domain verification, SPF/DKIM email auth, Let's Encrypt SSL challenges."],
        ["CNAME Apex", "N/A — not supported", "—", "Do NOT put CNAME on root apex. Use ANAME/ALIAS record (Cloudflare/Route53 specific)."],
        ["SRV", "Service location records", "_https._tcp.example.com 443", "Kubernetes service discovery, XMPP servers."],
        ["CAA", "Restrict which CAs can issue SSL", "0 issue \"letsencrypt.org\"", "Prevents unauthorized SSL certificate issuance for your domain."],
      ]}
    />,
    ...
    <p key="ttl-sub" className="text-muted-foreground mb-6">
      TTL (Time To Live) is set on each DNS record in seconds. It tells every Recursive Resolver on earth how long to cache your answer before asking again. This single number is the root cause of <em>all</em> "why isn't my DNS change live yet?" frustration.
    </p>,
    ...
    <p key="cdn-sub" className="text-muted-foreground mb-6">
      When you enable Cloudflare's orange-cloud proxy on a record, DNS stops being just a lookup system. The A record no longer points to your server — it points to <strong>Cloudflare's edge network</strong>. Your real server IP becomes completely hidden from the public internet.
    </p>,
...
# ^ Cloudflare's IP. Your server is invisible.`}
    ...
    <Callout key="cdn-callout" type="info" title="Cloudflare Proxied Records Break Some Things">
      When a record is proxied, only HTTP/HTTPS (ports 80/443) traffic is forwarded to your origin. If your app uses a custom TCP port (e.g., a game server on port 25565 or a DB on 5432), it will silently fail. Use <strong>DNS-Only</strong> mode for non-HTTP services.
    </Callout>,
    ...
    <p key="dnssec-sub" className="mb-4 text-muted-foreground">
      Standard DNS is completely unauthenticated. A <Highlight variant="warning">Man-in-the-Middle attacker</Highlight> on the same network as a resolver can intercept a query and inject a fake response — redirecting <code>yourbank.com</code> to a malicious clone. This is called <strong>DNS Cache Poisoning</strong>. DNSSEC fixes this.
    </p>,
    ...
    <Table
      ...
      rows={[
        ["dig app.myshop.com", "Queries your configured resolver for a record.", "First debug step. Shows the answer + TTL remaining."],
        ["dig @8.8.8.8 app.myshop.com", "Queries Google's resolver specifically.", "Check if a record has propagated to a specific resolver."],
        ["dig app.myshop.com +trace", "Traces the full recursive resolution path.", "See exactly which Root → TLD → Authoritative servers are being hit."],
        ...
        ["whois myshop.com", "Shows Registrar, expiry date, NS records.", "Check who owns a domain and when it expires."],
      ]}
    />,
    <CodeBlock
      key="dig-example"
...
# TTL=300: This resolver will cache this answer for 5 minutes`}
    ...
    <MistakeCard
      ...
      solution={<>Propagation is limited by TTL. Use <code>dig @8.8.8.8 domain.com</code> and <code>dig @1.1.1.1 domain.com</code> to check specific resolvers. Use <code>dnschecker.org</code> to see global propagation status visually.</>}
    />,
    <MistakeCard
      key="m4"
      number={4}
    ...
    <Callout key="perf-callout" type="tip" title="Browser Hint: dns-prefetch & preconnect">
      DNS resolution takes 20–120ms per domain. If your page loads resources from a third-party domain (Google Fonts, Stripe, Segment), the browser has to resolve that domain's DNS before fetching anything. You can eliminate this latency by hinting the browser to resolve early:
      <br /><br />
      <code>&lt;link rel="dns-prefetch" href="https://fonts.googleapis.com" /&gt;</code> — Resolves DNS in background before the resource is needed.<br />
      <code>&lt;link rel="preconnect" href="https://fonts.googleapis.com" /&gt;</code> — Goes further: DNS + TCP + TLS handshake, all before the CSS even parses.
    </Callout>,
  ],
};

...
export const acidPropertiesTopic: Topic = {
	...
	content: [
		<p key="1">
			If a user initiates a $50 bank transfer, the database mathematically performs two commands: `UPDATE user1 = user1
			- 50` and `UPDATE user2 = user2 + 50`. What happens if the physical power cable to the server sparks and violently
			trips immediately after line 1? User 1 lost $50 permanently, and User 2 never received it. The money evaporated.
		</p>,
		<p key="2" className="mt-4">
			<strong>ACID</strong> is a set of iron-clad guarantees implemented directly into the deepest core of PostgreSQL
			and MySQL to prevent these architectural nightmares.
		</p>,
		...
		<Callout key="5" type="warning" title="NoSQL Abandons ACID (Historically)">
			To achieve massive horizontal cloud scale in the 2010s, databases like Cassandra and early MongoDB historically
			abandoned the 'I' (Isolation) or 'C' (Consistency) to run blazing fast. This is exactly why 99% of global massive
			banking and fintech institutions strictly use rigid SQL Postgres arrays for their core ledgers, avoiding NoSQL for
			financial transactions entirely.
		</Callout>,
		<h3 key="6" className="text-xl font-bold mt-8 mb-4">
			Isolation Levels: The Hidden Complexity
		</h3>,
		<p key="7">
			The "I" in ACID isn't binary. SQL databases offer multiple <strong>Isolation Levels</strong>&nbsp;that trade
		...
		<Table
			...
			rows={[
				[
					...
					"Dirty Reads (reading data that gets rolled back).",
				],
				[
					"Read Committed",
					"Only reads committed data. Default in PostgreSQL.",
					"Non-Repeatable Reads (same query returns different results).",
				],
				[
					"Repeatable Read",
					"Guarantees same query returns same result within a transaction.",
					"Phantom Reads (new rows can appear).",
				],
				[
					"Serializable",
					"Full isolation. Transactions execute as if they ran one-at-a-time.",
					"Slowest. Can cause deadlocks.",
				],
			]}
		/>,
		<Callout key="9" type="info" title="Modern NoSQL Evolution">
			Modern NoSQL databases have evolved. MongoDB 4.0+ supports multi-document ACID transactions. DynamoDB offers{" "}
			<strong>Transactions</strong>&nbsp;for up to 25 items. The line between SQL and NoSQL is blurring.
		</Callout>,
		...
		<p key="11" className="mb-4">
			How does a database guarantee <strong>Durability</strong>&nbsp;without destroying performance? The secret is{" "}
			<strong>Write-Ahead Logging</strong>&nbsp;(WAL). Before modifying any data page in memory, the database first
			writes the change to a sequential append-only log file on disk.
		</p>,
		...
		<p key="14" className="mb-4">
			<strong>Isolation</strong>&nbsp;isn't free. To prevent race conditions, databases use <strong>locks</strong>. When
			Transaction A reads a row, it acquires a <strong>Shared Lock</strong>. When Transaction B tries to update that
			same row, it must wait for a <strong>Exclusive Lock</strong>, creating contention.
		</p>,
		<Table
			key="15"
		...
		<Callout key="16" type="danger" title="Real-World War Story: The Accidental Table Lock">
			In 2019, a developer ran <code>ALTER TABLE users ADD COLUMN preferences JSON</code>&nbsp;on a production
			PostgreSQL database with 50 million users. The migration acquired an <strong>ACCESS EXCLUSIVE</strong>&nbsp;lock,
			blocking all reads and writes for 8 minutes. The entire application went down. Lesson: Use tools like{" "}
			<code>pg_repack</code>&nbsp;or <code>pt-online-schema-change</code>&nbsp;for zero-downtime migrations.
		</Callout>,
		...
		<p key="18" className="mb-4">
			PostgreSQL uses <strong>Multi-Version Concurrency Control</strong>&nbsp;(MVCC) to allow reads and writes to happen
			simultaneously without blocking. Instead of locking rows, it keeps multiple versions of each row.
		</p>,
		<CodeBlock
			key="19"
...
-- This is "Repeatable Read" isolation in action`}
		...
		<p key="20" className="mt-4 text-sm text-muted-foreground">
			Each row has hidden metadata: <code>xmin</code>&nbsp;(transaction ID that created it) and <code>xmax</code>
			&nbsp;(transaction ID that deleted it). PostgreSQL uses these to determine which version of a row each transaction
			should see.
		</p>,
		...
		<p key="22" className="mb-4">
			Even with <strong>Repeatable Read</strong>, a subtle anomaly can occur: <strong>Phantom Reads</strong>. Your
			transaction reads a range of rows twice, but new rows appear in the second read because another transaction
			inserted them.
		</p>,
		<CodeBlock
			key="23"
...
WHERE user_id = 123 AND version = 5;  -- Fails if version changed`}
		...
		<Table
			...
			rows={[
				[
					"Analytics queries",
					"Use READ UNCOMMITTED or read replicas",
					"Slightly stale data (acceptable for dashboards)",
				],
				[
					"High-throughput logging",
					"Disable fsync (synchronous_commit = off)",
					"Last few seconds of logs lost on crash",
				],
				[
					"Idempotent operations",
					"Use eventual consistency (NoSQL)",
					"Temporary inconsistency (acceptable for likes/views)",
				],
		...
		<Callout key="31" type="warning" title="The fsync Controversy">
			PostgreSQL's <code>fsync</code>&nbsp;setting forces the OS to flush WAL writes to physical disk before returning
			success. Disabling it (<code>synchronous_commit = off</code>) can <strong>triple write throughput</strong>, but
			you risk losing the last few seconds of committed transactions during a crash. Only disable for non-critical data
			like session logs or metrics.
		</Callout>,
		...
		<p key="33" className="mb-4">
			In 2013, a cryptocurrency exchange suffered a catastrophic bug. Their custom database layer implemented
			"optimistic ACID"—it assumed transactions would succeed and only checked constraints <strong>after</strong>
			&nbsp;committing to disk. A race condition allowed two concurrent withdrawals to drain an account below zero.
		</p>,
		...
		<Callout key="35" type="tip" title="Pro Tip: Use SELECT FOR UPDATE for Critical Reads">
			When you need to read a value and then update it based on that value (like checking a balance before withdrawal),
			use <code>SELECT FOR UPDATE</code>&nbsp;to acquire an exclusive lock immediately. This prevents race conditions
			without requiring SERIALIZABLE isolation.
		</Callout>,
		<CodeBlock
			key="36"

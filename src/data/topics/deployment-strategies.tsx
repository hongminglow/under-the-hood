...
export const deploymentStrategiesTopic: Topic = {
	...
	description: "A deep dive into Rolling, Blue-Green, and Canary deployments for achieving zero-downtime releases.",
	tags: ["devops", "kubernetes", "cloud", "automation", "zero-downtime"],
	icon: "Rocket",
	content: [
		<p key="intro" className="mb-6">
			Deployment strategies dictate how new code replaces old code in a production environment. The end goal for modern
			cloud-native architectures is <strong>Zero-Downtime Deployment</strong>, meaning the end user experiences no
			service interruption (e.g., no 503 HTTP errors) while the application is updated.
		</p>,
		...
		<Table
			...
			rows={[
				...
				[
					"Canary",
					"Gradual (e.g., 5%, 10%, 100%)",
					"Low / Medium",
					"Fast (Stop directing traffic to Canary)",
					"Highest (Tests in prod)",
				],
			]}
		/>,
		...
		<p key="p-rolling" className="mb-4">
			A rolling deployment gradually replaces application instances with the new version. If you have 10 servers, the
			load balancer takes server #1 offline, deploys the new code to it, brings it back online, and then moves to server
			#2.
		</p>,
		...
		<p key="p-bluegreen" className="mb-4">
			This requires two identical environments. The "Blue" environment runs the current live version. The "Green"
			environment is provisioned with the new version. Once Green passes all health checks, the load balancer/router
		...
		<p key="p-canary" className="mb-4">
			Named after the "canary in a coal mine", this strategy routes a small percentage of total traffic (e.g., 5%) to
			the new version while the rest stays on the old version. Over time, observability metrics are monitored. If errors
		...
		<div key="callout-conclusion" className="mt-10">
			<Callout type="info" title="A Note on Database Migrations">
				For <i>any</i> zero-downtime deployment strategy, you cannot execute blocking database schema changes (like
				`ALTER TABLE... DROP COLUMN`). You must use the <strong>Expand-and-Contract</strong> pattern (add new column,
				sync data, migrate code to read/write new column, then eventually drop old column months later).
			</Callout>
		</div>,
	],
};

...
export const serverlessTopic: Topic = {
	...
	content: [
		<p key="1">
			First came physical servers. Then came Virtual Machines (EC2). Then came Containers (Docker). The final evolution
		...
		<Grid key="3" cols={2} gap={6} className="mb-8">
			<Card title="Traditional Servers">
				<ul className="text-sm space-y-2 list-disc pl-4 mt-2 text-muted-foreground">
					...
					<li>
						<strong>Billing:</strong> You pay by the hour, even if nobody visits your site at 3 AM.
					</li>
					<li>
						<strong>Scaling:</strong> Takes minutes to boot a new VM or container when traffic spikes.
					</li>
					<li>
						<strong>Overhead:</strong> OS updates, security patching, Dockerfiles.
					</li>
				</ul>
			</Card>
			<Card title="Serverless Functions">
				<ul className="text-sm space-y-2 list-disc pl-4 mt-2 text-muted-foreground">
					<li>
						<strong>Ephemeral:</strong> The function spins up instantly on a request, returns a response, and dies.
					</li>
					<li>
						<strong>Billing:</strong> You pay purely by the millisecond of execution time. (0 traffic = $0 bill).
					</li>
					<li>
						<strong>Scaling:</strong> Can handle 0 to 10,000 requests per second instantaneously.
					</li>
					<li>
		...
		<p key="5">
			...
			the runtime (Node.js/Python). This initial spin-up can take anywhere from 100ms to 2 seconds — a severe latency
			penalty known as a <strong>Cold Start</strong>. After the first request, the function stays "warm" in memory for a
			short period (~15 mins) to instantly process subsequent requests.
		</p>,
		<Callout key="6" type="tip" title="Edge Computing">
			V8 Isolates (used by Cloudflare Workers and Vercel Edge) solve cold starts by instantly loading pure JS functions
		...
		<Callout key="11" type="info" title="When to Use Serverless">
			Perfect for: Event-driven workloads (image processing on S3 upload), APIs with unpredictable traffic, scheduled
			tasks (cron jobs).
			<br />
			<br />
			Avoid for: Long-running processes, high-throughput sustained workloads (cheaper to run a dedicated server),

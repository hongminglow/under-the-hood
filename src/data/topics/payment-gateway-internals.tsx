...
export const paymentGatewayInternalsTopic: Topic = {
	...
	tags: ["payments", "security", "fintech", "pci-dss", "stripe", "tokenisation", "3ds"],
	icon: "CreditCard",
	content: [
		<p key="1">
			Every time a user clicks <strong>Pay Now</strong>, a sequence of encrypted messages races across at least six
			independent systems — your server, a payment gateway, a payment processor, two card networks, an issuing bank, and
			an acquiring bank — all in under <strong>two to three seconds</strong>. Almost none of this happens inside your
			own infrastructure. Understanding these layers is essential for any engineer building commerce features, because a
			single misstep exposes either cardholder data or millions in fraud liability.
		</p>,
		...
		<p key="5">
			There are three distinct phases. Most developers only think about authorisation, but all three matter for
			accounting, fraud, and user experience.
		</p>,
		...
		<p key="8">
			Raw PANs (e.g. <code>4111 1111 1111 1111</code>) are the most sensitive data in fintech. Tokenisation replaces
			them with a surrogate value that is meaningless to an attacker.
		</p>,
...
// stripe.com now holds the PAN; you store only paymentIntent.id`}
		...
		<Table
			key="12"
			headers={["Provider", "Type", "Best For", "Key Technical Differentiator"]}
			rows={[
				[
					"Stripe",
		...
		<Callout key="13" type="info" title="Why Stripe Can Be Both Gateway and Acquirer">
			Traditional setups required a separate payment gateway (to collect card data), a processor (to talk to card
			networks), and an acquiring bank (to hold the money). Stripe holds all three licenses in the US, EU, and more —
			this is why Stripe settlement happens in ~2 business days compared to 5+ days via a traditional processor chain.
			Adyen does the same at enterprise scale.
		</Callout>,
		...
		<p key="15">
			3DS2 is the modern replacement for the original 3D Secure (3DS1), which required a full browser redirect and a
			static password — a terrible UX that drove cart abandonment rates above 30%. 3DS2 is risk-based: only high-risk
			transactions get a challenge.
		</p>,
		...
		<p key="18">
			The <strong>Payment Card Industry Data Security Standard (PCI DSS)</strong> is a contractual requirement — not a
			law — mandated by the card networks for any entity that touches cardholder data. Version 4.0 (2022) is the current
			standard.
		</p>,
		...
		<Grid key="19" cols={2} gap={6}>
			...
			<Card title="Why Using Stripe Simplifies PCI Scope" description="SAQ A vs SAQ D">
				<p className="mt-2 text-sm text-muted-foreground leading-relaxed">
					...
					<strong>SAQ D</strong> — 329 requirements, extensive audits, and hardware security modules (HSMs).
				</p>
			</Card>
		</Grid>,
		...
		<Callout key="21" type="warning" title="These are the real attack surfaces">
			Payment infrastructure is the highest-value target on the internet. Understanding these vectors is essential for
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ received: true });
		...
		<p key="27">
			Behind the modern REST APIs of Stripe and Braintree lies an ancient, battle-hardened binary protocol:{" "}
			<strong>ISO 8583</strong>. This is the actual message format transmitted between processors and card networks.
			Every ATM withdrawal and every POS terminal swipe speaks ISO 8583.
		</p>,
		...
		<Callout key="29" type="tip" title="Why modern gateways exist">
			Before Stripe (2010), merchants had to implement ISO 8583 directly, sign lengthy contracts with acquiring banks,
			pass full PCI DSS Level 1 audits, and integrate with each card network separately. Stripe's genius was wrapping
			all of that behind two REST API calls and a publishable key. The ISO 8583 messages still fly under the hood —
			Stripe just never shows them to you.
		</Callout>,
		...
		<p key="31">
			Every card transaction carries invisible costs. Understanding interchange helps you make informed decisions about
			which payment methods to accept and why some gateways cost more than others.
		</p>,
		...
		<Table
			...
			rows={[
				...
				["Assessment / Scheme Fee", "Acquiring bank", "Card network (Visa/Mastercard)", "0.13%–0.15% of transaction"],
				["Processor / Gateway Markup", "Merchant", "Gateway/Processor (Stripe, Braintree)", "0.2%–0.5% + flat fee"],
				[
				...
				["Chargeback Fee", "Merchant", "Acquiring bank", "$15–$25 per dispute"],
			]}
		/>,

		<Callout key="33" type="info" title="Interchange++ Pricing (Enterprise)">
			At scale ($1M+/month), enterprises switch from blended pricing (2.9% + $0.30 flat) to{" "}
			<strong>Interchange++</strong>: they pay the exact raw interchange rate set by Visa/Mastercard + a fixed processor
			markup (e.g., 0.15% + $0.10). This can reduce effective rates by 0.5–1.0%, saving six figures annually on high
			volume. Adyen and Checkout.com specialise in this model.
		</Callout>,
		...
		<Grid key="35" cols={2} gap={6}>
			...
			<Card title="What You Must Never Do" description="Instant compliance violations">
				<ul className="list-disc pl-5 mt-2 text-sm text-muted-foreground space-y-1">
					...
					<li>
						Never expose your Stripe <strong>secret key</strong> in frontend code or public repos.
					</li>

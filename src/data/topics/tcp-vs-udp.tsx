...
export const tcpVsUdpTopic: Topic = {
  ...
  content: [
    ...
    <p key="5" className="mb-4">
      Before TCP can send a single byte of application data, it must complete a <strong>three-way handshake</strong>. This adds <Highlight variant="warning">1 full Round-Trip Time (RTT)</Highlight>&nbsp;to every new connection.
    </p>,
    <Flow
      key="6"
      steps={[
    ...
    <Callout key="7" type="warning" title="Real-World Impact">
      On a 100ms RTT connection, the handshake alone costs <strong>100ms</strong>&nbsp;before any data moves. For microservices making hundreds of requests per second, this compounds into seconds of wasted time. This is why <strong>connection pooling</strong>&nbsp;and <strong>HTTP keep-alive</strong>&nbsp;are critical optimizations.
    </Callout>,
    ...
    <p key="11" className="mb-4">
      TCP's reliability guarantee creates a critical bottleneck called <strong>Head-of-Line (HOL) Blocking</strong>. Because TCP guarantees in-order delivery, a single lost packet blocks <em>all subsequent packets</em>&nbsp;from being processed, even if they've already arrived.
    </p>,
    <CodeBlock
      key="12"
...
Result: 100ms delay for packets that were already there.`}
    ...
    <p key="13" className="mb-4">
      This is devastating for multiplayer games. Imagine your character's position update (Packet 3) being delayed because an unrelated chat message (Packet 2) was lost. UDP avoids this entirely by letting the application decide what to do with out-of-order data.
    </p>,
    ...
    <p key="15" className="mb-4">
      TCP includes sophisticated congestion control algorithms (<strong>Slow Start</strong>, <strong>Congestion Avoidance</strong>, <strong>Fast Retransmit</strong>) that automatically reduce transmission speed when packet loss is detected. This prevents network collapse but can be overly conservative.
    </p>,
    ...
    <Table
      key="23"
      headers={["Feature", "TCP (HTTP/2)", "QUIC (HTTP/3)"]}
      rows={[
    ...
    <Callout key="24" type="success" title="QUIC's Killer Feature">
      <strong>0-RTT Resumption</strong>: If you've connected to a server before, QUIC can send application data in the <em>very first packet</em>, with no handshake. This is impossible with TCP+TLS.
    </Callout>,
...
udpServer.bind(8081);`}
    ...
    <Callout key="35" type="info" title="The Future">
      The industry is converging on <strong>UDP + custom reliability</strong>. QUIC, WebRTC, and even game engines like Unreal Engine 5's replication system all follow this pattern. TCP's one-size-fits-all approach is being replaced by application-specific protocols that use UDP as a fast, flexible foundation.
    </Callout>,
  ],
};

...
export const awsServicesMappingTopic: Topic = {
	...
	content: [
		<p key="1">
			Amazon Web Services (AWS) is the largest cloud computing provider in the world. While you could technically rent
			raw servers (EC2) and install all your own software on them, the real value of AWS lies in its{" "}
			<strong>managed services</strong>. Instead of maintaining an open-source tool yourself, you pay AWS to handle the
			underlying infrastructure, high availability, scaling, and backups.
		</p>,
		<h3 key="2" className="text-xl font-bold mt-8 mb-4">
			Why Do We Pay For AWS Managed Services?
		</h3>,
		<p key="3">
			If a Redis server is free to download, why do companies pay AWS thousands of dollars a month for ElastiCache for
		...
		<Grid key="4" cols={2} gap={6} className="mt-4">
			...
			<FeatureCard icon={Database} title="Automated Backups" subtitle="Patching and point-in-time restore" theme="teal">
				<p className="mt-2 text-sm leading-relaxed text-teal-700 dark:text-teal-300/80">
		...
		<Callout key="7" type="tip" title="EC2 isn't exactly a 'managed' service">
			While EC2 provides the server dynamically, AWS only manages the hardware and hypervisor. You are still responsible
			for the OS, patching, security, and the application. This is called{" "}
			<strong>IaaS (Infrastructure as a Service)</strong>. Services like RDS and DynamoDB are{" "}
			<strong>PaaS (Platform)</strong> or <strong>SaaS (Software as a Service)</strong>, where AWS manages the software
			layers too.
		</Callout>,
		...
		<p key="9">
			A common paradigm shift is moving from running highly-available NGINX proxy nodes to using an AWS ALB.
		</p>,
		<Flow
			key="10"
			steps={[
		...
		<Grid key="13" cols={2} gap={6}>
			<FeatureCard icon={ServerCog} title="The Hard Way" subtitle="Self-hosted Postgres on EC2" theme="rose">
				<ul className="list-disc pl-5 mt-2 text-sm text-rose-700 dark:text-rose-300/80 space-y-1">
					...
					<li>
						If the primary dies, wake up at 3 AM to run `pg_ctl promote` on the replica to elect it as the new leader.
					</li>
			...
			<FeatureCard icon={Database} title="The AWS Way" subtitle="Amazon RDS managed service" theme="emerald">
				<ul className="list-disc pl-5 mt-2 text-sm text-emerald-700 dark:text-emerald-300/80 space-y-1">
					...
					<li>
						If primary dies, AWS automatically flips the DNS endpoint to the standby replica within 60 seconds. You
						sleep through it.
					</li>
				</ul>
			</FeatureCard>
		...
		<p key="15">
			The pinnacle of managed services on AWS is <strong>Serverless</strong>. This means there are no EC2 instances
			visible to you, no OS patching, no capacity planning, and you pay exactly for what you use (often per request).
		</p>,
		<Table
			key="16"
		...
		<Callout key="17" type="info" title="The Trade-off: Vendor Lock-in">
			The more heavily you rely on highly specific AWS managed services (like DynamoDB, SQS, or Lambda), the harder it
			becomes to move your application to Azure, Google Cloud, or on-premise later on. The industry consensus is usually
			that <strong>the speed and operational safety you gain far outweighs the cost of vendor lock-in</strong> for most
			companies.
		</Callout>,
	],
};

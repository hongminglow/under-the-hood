...
export const sqlVsNosqlTopic: Topic = {
  ...
  content: [
    <p key="1">
      For 30 years, Relational Databases (SQL) completely ran the world. In the
      2010s, developers realized that forcing dynamic modern web-app models into
      rigid 2D Excel-like tables was painful, giving rise to Document Databases
      (NoSQL).
    </p>,
    ...
    <Callout
      key="4"
      type="tip"
      title="The Scaling Myth (Horizontal vs Vertical)"
    >
      It is a known physics fact that joining 5 tables together is terribly
      slow, causing SQL databases to hit physical CPU limits (forcing expensive
      Vertical Scaling). NoSQL deliberately abandons relations entirely, which
      allows you to effortlessly split the database across 100 cheap servers
      (Horizontal Sharding) indefinitely without math conflicts.
    </Callout>,
    <h3 key="5" className="text-xl font-bold mt-8 mb-4">
      The Schema Flexibility Trade-off
    </h3>,
    <p key="6">
      SQL enforces a <strong>rigid schema</strong>. Every User row must have the
      exact same columns. Adding a new field requires an{" "}
      <code>ALTER TABLE</code> migration across millions of rows.
      <br />
      <br />
      NoSQL is <strong>schema-less</strong>. One Product document can have 50
      fields, another can have 5. Perfect for rapidly evolving startups, but
      dangerous for data integrity (no enforcement of required fields).
    </p>,
    <Table
      key="7"
      ...
      rows={[
        ...
        [
          "Cassandra",
          "NoSQL (Wide-Column)",
          "Massive write-heavy workloads (IoT, logs)",
        ],
        [
          "Redis",
          "NoSQL (Key-Value)",
    ...
    <Grid key="9" cols={2} gap={6} className="my-8">
      <Card title="Complex Relationships">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          queries. NoSQL would require multiple round-trips or massive data
          duplication.
        </p>
      </Card>
      ...
      <Card title="Structured, Stable Data">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          at the database level. Bad data physically cannot enter.
        </p>
      </Card>
      <Card title="Complex Queries & Analytics">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          query languages are primitive in comparison.
        </p>
      </Card>
    </Grid>,
    ...
    <Grid key="11" cols={2} gap={6} className="my-8">
      <Card title="Massive Scale (Horizontal)">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          databases hit scaling limits around 10-50 nodes due to JOIN
          complexity.
        </p>
      </Card>
      <Card title="Flexible/Evolving Schema">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          insert it. No migrations needed.
        </p>
      </Card>
      ...
      <Card title="High Write Throughput">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          millions of inserts/sec due to index maintenance overhead.
        </p>
      </Card>
    </Grid>,
    ...
    <p key="15" className="mb-4">
      NoSQL isn't "better"—it trades SQL's guarantees for scale. Here's what you
      lose:
    </p>,
    <Grid key="16" cols={2} gap={6} className="my-8">
      <Card title="Data Duplication Nightmare">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          amplification" and causes consistency issues.
        </p>
      </Card>
      ...
      <Card title="Eventual Consistency">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          <strong>eventual consistency</strong>—acceptable for social feeds,
          catastrophic for banking.
        </p>
      </Card>
      <Card title="Limited Query Capabilities">
        ...
        <p className="text-xs italic text-muted-foreground">
          ...
          to a SQL warehouse (Snowflake, BigQuery).
        </p>
      </Card>
    </Grid>,
    ...
    <p key="18" className="mb-4">
      In 2013-2015, MongoDB gained a reputation for <strong>data loss</strong>.
      The default configuration didn't wait for writes to be acknowledged by
      disk (<code>w:0</code>), meaning "success" responses were sent before data
      was durable. Server crashes = lost data.
    </p>,
...
class UserService {
  ...
  async logActivity(userId, action) {
    ...
    <Table
      ...
      rows={[
        [
          "PostgreSQL",
          "N/A (is SQL)",
          "JSONB columns (document storage), horizontal scaling (Citus)",
        ],
        [
          "MongoDB",
          "ACID transactions (4.0+), SQL-like aggregation pipeline",
          "N/A (is NoSQL)",
        ],
        [
          "CockroachDB",
    ...
    <Callout
      key="27"
      type="info"
      title="PostgreSQL's JSONB: Best of Both Worlds"
    >
      PostgreSQL's <code>JSONB</code>&nbsp;column type lets you store flexible
      JSON documents while keeping ACID guarantees and relational features. You
      can <code>JOIN</code>&nbsp;JSON columns with regular tables, index JSON
      fields, and query with SQL. This is why many "NoSQL" use cases now use
      PostgreSQL instead.
    </Callout>,
      ...
      steps={[
        {
          ...
          description:
            "Highly relational (social graph, e-commerce)? → SQL. Document-centric (CMS, catalogs)? → NoSQL",
        },
        {
          title: "2. Check Transaction Requirements",
          description:
            "Need ACID guarantees (payments, inventory)? → SQL. Eventual consistency OK? → NoSQL",
        },
        {
          title: "3. Estimate Scale",
          description:
            "< 10M records, < 10k req/sec? → SQL. Billions of records, 100k+ writes/sec? → NoSQL",
        },
        {
          title: "4. Evaluate Query Complexity",
          description:
            "Complex JOINs, analytics, aggregations? → SQL. Simple key-value lookups? → NoSQL",
        },
        {
          title: "5. Consider Team Expertise",
          description:
            "Team knows SQL well? → SQL. Learning curve acceptable? → NoSQL",
        },
      ]}
    />,
    ...
    <Callout key="32" type="tip" title="Pro Tip: Start SQL, Add NoSQL Later">
      The best strategy: start with PostgreSQL for everything. As you scale,
      identify bottlenecks and add specialized databases (Redis for caching,
      Elasticsearch for search, Cassandra for logs). This is called{" "}
      <strong>polyglot persistence</strong>&nbsp;and is how companies like Uber
      and Netflix operate at scale.
    </Callout>,
    ...
    <p key="34" className="mb-4">
      <strong>NewSQL</strong>&nbsp;databases (CockroachDB, Google Spanner,
      YugabyteDB) promise the impossible: SQL's ACID guarantees with NoSQL's
      horizontal scalability. They use distributed consensus algorithms (Raft,
      Paxos) to maintain consistency across 1000+ nodes.
    </p>,

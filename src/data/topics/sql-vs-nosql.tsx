...
export const sqlVsNosqlTopic: Topic = {
	...
	content: [
		<p key="1">
			For 30 years, Relational Databases (SQL) completely ran the world. In the 2010s, developers realized that forcing
			dynamic modern web-app models into rigid 2D Excel-like tables was painful, giving rise to Document Databases
			(NoSQL).
		</p>,
		...
		<Grid key="3" cols={2} gap={6} className="my-8">
			<FeatureCard icon={Database} title="SQL" subtitle="relational structure and ACID safety" theme="amber">
				...
				<p className="text-sm font-semibold mt-3 text-amber-700 dark:text-amber-400">Best for:</p>
				<ul className="list-disc pl-5 text-sm text-amber-700 dark:text-amber-100/75 mt-1 space-y-1">
			...
			<FeatureCard icon={Braces} title="NoSQL" subtitle="document shape and horizontal scale" theme="cyan">
				...
				<p className="text-sm font-semibold mt-3 text-cyan-700 dark:text-cyan-400">Best for:</p>
				<ul className="list-disc pl-5 text-sm text-cyan-700 dark:text-cyan-100/75 mt-1 space-y-1">
		...
		<Callout key="4" type="tip" title="The Scaling Myth (Horizontal vs Vertical)">
			...
			CPU limits (forcing expensive Vertical Scaling). NoSQL deliberately abandons relations entirely, which allows you
			to effortlessly split the database across 100 cheap servers (Horizontal Sharding) indefinitely without math
			conflicts.
		</Callout>,
		...
		<p key="6">
			SQL enforces a <strong>rigid schema</strong>. Every User row must have the exact same columns. Adding a new field
			requires an <code>ALTER TABLE</code> migration across millions of rows.
			<br />
			<br />
			NoSQL is <strong>schema-less</strong>. One Product document can have 50 fields, another can have 5. Perfect for
			rapidly evolving startups, but dangerous for data integrity (no enforcement of required fields).
		</p>,
		<Table
			key="7"
		...
		<Grid key="9" cols={2} gap={6} className="my-8">
			<FeatureCard icon={Database} title="Complex Relationships" subtitle="SQL strength" theme="amber">
				<p className="text-sm text-amber-700 dark:text-amber-400 mb-2">Many-to-many relationships, deep JOINs.</p>
			...
			<FeatureCard icon={Database} title="ACID Transactions" subtitle="SQL strength" theme="amber">
				<p className="text-sm text-amber-700 dark:text-amber-400 mb-2">Financial operations, inventory management.</p>
				<p className="text-sm text-amber-700 dark:text-amber-100/75">
					Banking (transfer $100 from A to B atomically), payment processing, stock trading. SQL guarantees{" "}
					<strong className="text-amber-700 dark:text-amber-400">atomicity</strong>: either all operations succeed or all fail. NoSQL historically sacrificed this for
			...
			<FeatureCard icon={Database} title="Structured, Stable Data" subtitle="SQL strength" theme="amber">
				<p className="text-sm text-amber-700 dark:text-amber-400 mb-2">Schema rarely changes, data integrity critical.</p>
				<p className="text-sm text-amber-700 dark:text-amber-100/75">
			...
			<FeatureCard icon={Database} title="Complex Queries & Analytics" subtitle="SQL strength" theme="amber">
				<p className="text-sm text-amber-700 dark:text-amber-400 mb-2">Aggregations, window functions, CTEs.</p>
				<p className="text-sm text-amber-700 dark:text-amber-100/75">
					Business intelligence dashboards, reporting systems. SQL's query language is <strong className="text-amber-700 dark:text-amber-400">50 years mature</strong>
		...
		<Grid key="11" cols={2} gap={6} className="my-8">
			<FeatureCard icon={Braces} title="Massive Scale (Horizontal)" subtitle="NoSQL strength" theme="cyan">
				<p className="text-sm text-cyan-700 dark:text-cyan-400 mb-2">Billions of records, petabytes of data.</p>
				<p className="text-sm text-cyan-700 dark:text-cyan-100/75">
					...
					databases like Cassandra can scale to <strong className="text-cyan-700 dark:text-cyan-400">1000+ nodes</strong>&nbsp;linearly. SQL databases hit scaling
			...
			<FeatureCard icon={Braces} title="Flexible/Evolving Schema" subtitle="NoSQL strength" theme="cyan">
				<p className="text-sm text-cyan-700 dark:text-cyan-400 mb-2">Rapid prototyping, frequent schema changes.</p>
				<p className="text-sm text-cyan-700 dark:text-cyan-100/75">
			...
			<FeatureCard icon={Braces} title="Document-Centric Data" subtitle="NoSQL strength" theme="cyan">
				<p className="text-sm text-cyan-700 dark:text-cyan-400 mb-2">Self-contained entities, minimal relationships.</p>
				<p className="text-sm text-cyan-700 dark:text-cyan-100/75">
			...
			<FeatureCard icon={Braces} title="High Write Throughput" subtitle="NoSQL strength" theme="cyan">
				<p className="text-sm text-cyan-700 dark:text-cyan-400 mb-2">Write-heavy workloads, append-only logs.</p>
				<p className="text-sm text-cyan-700 dark:text-cyan-100/75">
					Time-series data (metrics, logs, events), real-time analytics, clickstream tracking. Cassandra and DynamoDB
					are optimized for <strong className="text-cyan-700 dark:text-cyan-400">write amplification</strong>. SQL databases struggle with millions of inserts/sec
		...
		<Table
			...
			rows={[
				[
					"Stripe",
					"PostgreSQL (SQL)",
					"Financial transactions require ACID guarantees. Complex fraud detection queries need JOINs.",
					"Billions of transactions, 99.999% uptime",
				],
				[
					"Netflix",
					"Cassandra (NoSQL)",
					"Massive write throughput for viewing history (400M users). Horizontal scaling to 1000+ nodes.",
					"1 trillion writes/day",
				],
				[
					"Uber",
					"PostgreSQL + Redis",
					"SQL for transactional data (trips, payments). Redis for real-time location caching.",
					"10M trips/day",
				],
				[
					"Facebook",
					"MySQL + Cassandra",
					"MySQL for social graph (friends, posts). Cassandra for messaging (90B messages/day).",
					"2.9B users",
				],
				[
					"Airbnb",
					"PostgreSQL (SQL)",
					"Complex search queries (location, price, availability). Relational data (users, listings, bookings).",
					"7M listings globally",
				],
				[
					"Discord",
					"Cassandra (NoSQL)",
					"Billions of messages/day. Horizontal scaling for 150M+ users. Eventual consistency acceptable.",
					"1B+ messages/day",
				],
				[
					"GitHub",
					"MySQL (SQL)",
					"Code repositories, pull requests, issues are highly relational. ACID for git operations.",
					"100M+ repositories",
				],
				[
					"Twitter",
					"Manhattan (NoSQL)",
					"Custom distributed database for tweets. 500M tweets/day require massive write throughput.",
					"500M tweets/day",
				],
			]}
		/>,
		...
		<Grid key="16" cols={2} gap={6} className="my-8">
			<Card title="Data Duplication Nightmare">
				<p className="text-sm text-muted-foreground mb-2">Denormalization = copying data everywhere.</p>
				<p className="text-xs italic text-muted-foreground">
					In SQL, a user's email exists once. In NoSQL, if you embed user data in 1000 documents (orders, comments,
					posts), updating the email requires <strong>1000 writes</strong>. This is called "write amplification" and
					causes consistency issues.
				</p>
			</Card>
			<Card title="No Foreign Key Constraints">
				<p className="text-sm text-muted-foreground mb-2">Referential integrity enforced in application code.</p>
				<p className="text-xs italic text-muted-foreground">
					SQL prevents orphaned records (can't delete a user if orders reference them). NoSQL doesn't care—you can
					delete a user and leave 1000 orders pointing to a non-existent user. Your app must handle this.
				</p>
			</Card>
			<Card title="Eventual Consistency">
				<p className="text-sm text-muted-foreground mb-2">Reads might return stale data.</p>
				<p className="text-xs italic text-muted-foreground">
					In distributed NoSQL (Cassandra, DynamoDB), writes propagate across nodes over seconds. A user updates their
					profile, but reads from another node show old data. This is <strong>eventual consistency</strong>—acceptable
					for social feeds, catastrophic for banking.
				</p>
			</Card>
			<Card title="Limited Query Capabilities">
				<p className="text-sm text-muted-foreground mb-2">No JOINs, limited aggregations.</p>
				<p className="text-xs italic text-muted-foreground">
					MongoDB's aggregation pipeline is powerful but nowhere near SQL's expressiveness. Complex analytics queries
					(recursive CTEs, window functions, lateral joins) are impossible. You'll need to export data to a SQL
		...
		<p key="18" className="mb-4">
			In 2013-2015, MongoDB gained a reputation for <strong>data loss</strong>. The default configuration didn't wait
			for writes to be acknowledged by disk (<code>w:0</code>), meaning "success" responses were sent before data was
			durable. Server crashes = lost data.
		</p>,
		<Grid key="19" cols={2} gap={6} className="my-8">
			...
			<Card title="How MongoDB Fixed It">
				<ul className="text-xs text-muted-foreground space-y-1 list-disc pl-4">
					<li>
						Changed default to <code>w:1</code>&nbsp;(wait for primary acknowledgment)
					</li>
    ...
    return await cassandra.execute(
      'INSERT INTO activity_log (user_id, action, timestamp) VALUES (?, ?, ?)'
		...
		<Callout key="27" type="info" title="PostgreSQL's JSONB: Best of Both Worlds">
			PostgreSQL's <code>JSONB</code>&nbsp;column type lets you store flexible JSON documents while keeping ACID
			guarantees and relational features. You can <code>JOIN</code>&nbsp;JSON columns with regular tables, index JSON
			fields, and query with SQL. This is why many "NoSQL" use cases now use PostgreSQL instead.
		</Callout>,
		...
		<Flow
			...
			steps={[
				{
					title: "1. Analyze Data Relationships",
					description: "Highly relational (social graph, e-commerce)? → SQL. Document-centric (CMS, catalogs)? → NoSQL",
				},
				{
					title: "2. Check Transaction Requirements",
					description: "Need ACID guarantees (payments, inventory)? → SQL. Eventual consistency OK? → NoSQL",
				},
				{
					title: "3. Estimate Scale",
					description: "< 10M records, < 10k req/sec? → SQL. Billions of records, 100k+ writes/sec? → NoSQL",
				},
				{
					title: "4. Evaluate Query Complexity",
					description: "Complex JOINs, analytics, aggregations? → SQL. Simple key-value lookups? → NoSQL",
				},
				{
					title: "5. Consider Team Expertise",
					description: "Team knows SQL well? → SQL. Learning curve acceptable? → NoSQL",
				},
			]}
		/>,

...
export const nginxUnderTheHoodTopic: Topic = {
	...
	description:
		"An incredibly fast, event-driven web server, reverse proxy, and load balancer. Discover how it works and when to use bare-metal vs managed AWS options.",
	tags: ["infrastructure", "networking", "devops", "backend", "web-server"],
	icon: "Server",
	content: [
		<p key="intro" className="mb-6">
			<Highlight variant="primary">NGINX is not just a web server.</Highlight> While historically built to serve static
			HTML files faster than Apache (solving the C10K problem), modern NGINX fundamentally acts as the ultimate "traffic
		...
		<Grid key="capabilities-grid" cols={2} gap={6} className="mb-8">
			<Card title="Reverse Proxy">
				<p className="text-sm text-muted-foreground">
					Sitting in front of your Node.js/Python backend. It intercepts requests, handles HTTPS, and then passes raw
					HTTP to your internal app so your backend doesn't have to handle security overhead.
				</p>
			</Card>
			<Card title="Load Balancing">
				<p className="text-sm text-muted-foreground">
					Spreading incoming traffic across identical background servers using algorithms like Round Robin, Least
					Connections, or IP Hash.
				</p>
			</Card>
			<Card title="Static File Serving">
				<p className="text-sm text-muted-foreground">
					Blazing fast at returning static assets (images, CSS, JS) directly from the disk without ever waking up your
					expensive application logic.
				</p>
			</Card>
			<Card title="Caching & Rate Limiting">
				<p className="text-sm text-muted-foreground">
					Can temporarily store backend responses (microcaching) and choke overly aggressive clients to prevent DDoS
					attacks.
				</p>
			</Card>
		</Grid>,
			...
			steps={[
				{
					...
					description:
						"Reads and evaluates your nginx.conf. Binds to ports (e.g., 80/443), and orchestrates the worker processes.",
				},
				{
					title: "2. Worker Processes",
					description:
						"NGINX usually spawns one worker per CPU core. These are single-threaded and use asynchronous, non-blocking I/O (like JS Event Loop) to handle thousands of connections simultaneously.",
				},
				{
					title: "3. Connection Processing",
					description:
						"When a request arrives, the worker doesn't spawn a new thread. It simply handles the event, passes it to the backend, and is immediately free to serve the next user while waiting.",
				},
			]}
		/>,
		...
		<p key="setup-desc" className="mb-6">
			A major architectural decision is whether to physically spin up a standard NGINX server (via{" "}
			<code>nginx.conf</code> on EC2/Docker) or pay AWS to do it via an Application Load Balancer (ALB) or API Gateway.
		</p>,
		...
		<p key="deepdive-text" className="mb-4">
			If you choose the traditional route, NGINX is controlled entirely by a single nested configuration file. It relies
			on a hierarchy of "blocks": <code>http</code> → <code>server</code> → <code>location</code>.
		</p>,
    ...
    server {
		...
		<Grid key="pitfalls-grid" cols={1} gap={6} className="mb-8">
			<Card title="The 'Lost Client IP' Problem">
				<p className="text-sm text-slate-700 dark:text-slate-400 mb-3">
					<strong className="text-red-700 dark:text-red-400">The Problem:</strong> When the Node.js or Python backend logs a user's IP, it records the internal IP
				...
				<ul className="text-sm text-slate-700 dark:text-slate-400 list-disc pl-5 space-y-2">
					<li>
						<strong className="text-emerald-700 dark:text-emerald-400">Forward the real client IP:</strong> Ensure NGINX utilizes <code>proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;</code>.
					</li>
					<li>
						<strong className="text-emerald-700 dark:text-emerald-400">Trust the proxy explicitly:</strong> Configure your Express/backend framework to trust the proxy (e.g., <code>app.set('trust proxy', 1)</code> in
			...
			<Card title="Buffer Overflows via Large Uploads">
				<p className="text-sm text-slate-700 dark:text-slate-400 mb-3">
					<strong className="text-red-700 dark:text-red-400">The Problem:</strong> Users attempting to upload a 50MB video file get instantly rejected by NGINX
					with a <strong>413 Request Entity Too Large</strong> error, before the request even reaches the backend API.
				</p>
				<p className="text-xs uppercase tracking-[0.2em] text-slate-500 mb-2">What to do instead</p>
				<ul className="text-sm text-slate-700 dark:text-slate-400 list-disc pl-5 space-y-2">
					<li><strong className="text-emerald-700 dark:text-emerald-400">Understand the default guardrail:</strong> NGINX aggressively protects backends by defaulting max body sizes to 1MB.</li>
					<li>
						<strong className="text-emerald-700 dark:text-emerald-400">Raise the limit where needed:</strong> Explicitly override this limit inside your server or location block via{" "}
		...
		<Callout key="callout-conclusion" type="tip" title="When to use What?">
			If you are a solo developer or a mid-sized startup on a tight budget, manually deploying NGINX via Docker Compose
			or a cheap VPS is significantly cost-effective. For enterprise systems orchestrating massive Kubernetes clusters,
			you frequently offload this to Managed AWS Load Balancers or use NGINX Ingress Controllers to handle dynamic

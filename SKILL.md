...
1. **Semantic Contrasts (Yes/No, Pros/Cons):** When comparing opposing concepts (e.g., "With vs Without", "Good vs Bad"), use semantic themes to visually convey meaning:
   - Vulnerable / Cons / Errors: `theme="rose"` or `theme="slate"`
    - Secure / Pros / Success: prefer `theme="emerald"`, `theme="teal"`, or `theme="cyan"` for FeatureCards, or use a `type="success"` Callout when the green success treatment is desired.
...
2. **Neutral Progressions (Lists, Actors):** When displaying a neutral list of concepts (like DNS Actors) in a `<Grid>`, alternate the `theme` prop with clearly separated identities to break monotony and create depth without breaking the dark theme.
   - Preferred first-8 actor palette for high-contrast comparison pages: `"emerald"`, `"amber"`, `"cyan"`, `"sky"`, `"indigo"`, `"violet"`, `"fuchsia"`, `"orange"`.
   ...
   - Light green is still allowed for tags, default surfaces, and content where it already reads well.
   - Do not place visually adjacent actors on near-identical green/cyan/teal tones when the reader must compare them. Prefer stronger contrast pairs such as emerald vs amber, cyan vs violet, or sky vs amber.
